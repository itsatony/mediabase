...
from ..utils.gene_matcher import (
    normalize_gene_symbol,
    match_genes_bulk,
    get_gene_match_stats,
)
...
class ChemblDrugProcessor(BaseProcessor):
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.max_phase_cutoff = config.get(
            "max_phase_cutoff", 0
        )  # 0 = include all, 1+ = only drugs with specified phase or higher
        ...
        self.chembl_api_base = config.get(
    ...
    def download_chembl_data(self) -> Tuple[Path, Path]:
        ...
        try:
            ...
            if not chembl_dump.exists() or self.force_download:
                self.logger.info(
                    f"Downloading ChEMBL database dump from {self.chembl_db_url}"
                )
                chembl_dump = self.download_file(
                    url=self.chembl_db_url, file_path=chembl_dump
                )
            else:
            ...
            if not uniprot_mapping.exists() or self.force_download:
                self.logger.info(
                    f"Downloading ChEMBL-UniProt mapping from {self.chembl_mapping_url}"
                )
                uniprot_mapping = self.download_file(
                    url=self.chembl_mapping_url, file_path=uniprot_mapping
                )
            else:
                self.logger.info(
                    f"Using cached ChEMBL-UniProt mapping at {uniprot_mapping}"
                )
    ...
    def _create_temp_database(self, db_name: str) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Dropping existing temporary database {db_name} if present"
            )
            ...
            result = subprocess.run(
                create_cmd, env=env, check=True, capture_output=True, text=True
            )
    ...
    def _restore_dump_to_temp_db(self, dump_file: Path, temp_db_name: str) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Restoring ChEMBL dump {dump_file} to {temp_db_name} (this may take several minutes)"
            )
            ...
            if result.returncode != 0:
                ...
                if "error:" in stderr_lower or "fatal:" in stderr_lower:
                    self.logger.error(
                        f"pg_restore failed with errors: {result.stderr[:500]}"
                    )
                    return False
                else:
                    self.logger.warning(
                        f"pg_restore completed with warnings: {result.stderr[:200]}"
                    )
    ...
    def _query_temp_database(
        self, temp_db_name: str, query: str, params: Optional[tuple] = None
    ...
    def extract_chembl_dump(self, dump_file: Path) -> Path:
        ...
        try:
            ...
            if self.force_download and self.extracted_dir.exists():
                self.logger.info(
                    f"Clearing previous extractions at {self.extracted_dir}"
                )
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if not dmp_files:
                    raise ProcessingError(
                        "No .dmp file found in ChEMBL archive. Expected PostgreSQL custom format dump."
                    )
                ...
                temp_db_name = (
                    f"chembl_temp_{CHEMBL_VERSION}_{int(datetime.now().timestamp())}"
                )
                ...
                try:
                    ...
                    if not self._create_temp_database(temp_db_name):
                        raise ProcessingError(
                            "Failed to create temporary database for ChEMBL extraction"
                        )

                    # Restore dump to temporary database
                    if not self._restore_dump_to_temp_db(dmp_file, temp_db_name):
                        raise ProcessingError(
                            "Failed to restore ChEMBL dump to temporary database"
                        )
                    ...
                    self.logger.info(
                        f"Extracting {len(tables_of_interest)} tables from temporary database"
                    )
                    ...
                    for table_name, columns in tables_of_interest.items():
                        try:
                            ...
                            if df is not None and not df.empty:
                                ...
                                self.logger.info(
                                    f"Extracted {table_name}: {len(df)} rows → {output_file}"
                                )
                            else:
                                self.logger.warning(
                                    f"Table {table_name} is empty or query failed"
                                )
    ...
    def process_uniprot_mapping(self, mapping_file: Path) -> Dict[str, Set[str]]:
        ...
        try:
            ...
            self.logger.info(
                f"Processed {len(uniprot_to_chembl)} UniProt IDs mapped to ChEMBL targets"
            )
            return uniprot_to_chembl
    ...
    def create_optimized_tables(self) -> None:
        ...
        try:
            ...
            exists_result = self._safe_execute(
                f"SELECT EXISTS(SELECT 1 FROM pg_namespace WHERE nspname = '{schema_name}')"
            )
            ...
            if (
                self.db_manager
                and self.db_manager.conn
                and not getattr(self.db_manager.conn, "closed", True)
            ):
    ...
    def _process_molecule_dictionary(self, extracted_dir: Path) -> None:
        ...
        try:
            ...
            if existing_count > 0 and not self.force_download:
                self.logger.info(
                    f"Found {existing_count} existing drugs. Use force_download=True to reimport."
                )
                return
            ...
            if not molecule_dict_path.exists():
                self.logger.warning(
                    "molecule_dictionary.csv not found, skipping drug processing"
                )
                return
            ...
            if self.max_phase_cutoff > 0:
                ...
                df_molecules = df_molecules[
                    df_molecules["max_phase"] >= self.max_phase_cutoff
                ]
                self.logger.info(
                    f"Filtered to {len(df_molecules)} molecules with max_phase >= {self.max_phase_cutoff} (from {initial_count})"
                )
            ...
            if structures_path.exists():
                ...
                df_molecules = df_molecules.merge(
                    df_structures, on="molregno", how="left"
                )
            ...
            if properties_path.exists():
                ...
                df_molecules = df_molecules.merge(
                    df_properties, on="molregno", how="left"
                )
            ...
            try:
                for start_idx in range(0, len(df_molecules), batch_size):
                    ...
                    for _, row in batch.iterrows():
                        ...
                        structure_info = {}
                        if "canonical_smiles" in row and pd.notna(
                        ...
                        if "standard_inchi_key" in row and pd.notna(
                        ...
                        batch_data.append(
                            {
                                ...
                                "name": str(row["pref_name"])
                                if pd.notna(row.get("pref_name"))
                                else None,
                                "synonyms": synonyms[:10],  # Limit to first 10 synonyms
                                "max_phase": float(row["max_phase"])
                                if pd.notna(row.get("max_phase"))
                                else None,
                                "drug_type": str(row["molecule_type"])
                                if pd.notna(row.get("molecule_type"))
                                else None,
                                "molecular_weight": float(row["mw_freebase"])
                                if "mw_freebase" in row and pd.notna(row["mw_freebase"])
                                else None,
                                "atc_codes": [],  # Will be populated later if needed
                                "structure_info": json.dumps(structure_info)
                                if structure_info
                                else None,
                ...
                self._safe_commit()
                self.logger.info(
                    f"Processed molecule_dictionary: inserted {inserted_count} drugs"
                )
    ...
    def _process_drug_targets(self, extracted_dir: Path) -> None:
        ...
        try:
            ...
            if existing_count > 0 and not self.force_download:
                self.logger.info(
                    f"Found {existing_count} existing targets. Use force_download=True to reimport."
                )
                return
            ...
            if not target_dict_path.exists():
                self.logger.warning(
                    "target_dictionary.csv not found, skipping target processing"
                )
                return
            ...
            if TARGET_TYPES_OF_INTEREST:
                ...
                df_targets = df_targets[
                    df_targets["target_type"].isin(TARGET_TYPES_OF_INTEREST)
                ]
                self.logger.info(
                    f"Filtered to {len(df_targets)} targets of interest (from {initial_count})"
                )
            ...
            if sequences_path.exists():
                ...
                df_targets = df_targets.merge(
                    df_sequences, on="component_id", how="left"
                )
            ...
            if not mechanisms_path.exists():
                self.logger.warning(
                    "drug_mechanism.csv not found, creating targets without drug links"
                )
            ...
            molregno_to_chembl = {
                row[0]: row[1] for row in self.db_manager.cursor.fetchall()
            }

            self.logger.info(
                f"Processing {len(df_targets)} target records for insertion"
            )
            ...
            try:
                for start_idx in range(0, len(df_targets), batch_size):
                        ...
                        batch_data.append(
                            {
                                ...
                                if "mechanism_of_action" in row
                                and pd.notna(row["mechanism_of_action"])
                                else None,
                    ...
                    batch_data_valid = [
                        d for d in batch_data if d["chembl_id"] is not None
                    ]
                ...
                self._safe_commit()
                self.logger.info(
                    f"Processed drug_targets: inserted {inserted_count} target relationships"
                )
    ...
    def _process_drug_indications(self, extracted_dir: Path) -> None:
        ...
        try:
            ...
            if existing_count > 0 and not self.force_download:
                self.logger.info(
                    f"Found {existing_count} existing indications. Use force_download=True to reimport."
                )
                return
            ...
            if not indication_path.exists():
                self.logger.warning(
                    "drug_indication.csv not found, skipping indication processing"
                )
                return
            ...
            if self.max_phase_cutoff > 0:
                ...
                df_indications = df_indications[
                    df_indications["max_phase_for_ind"] >= self.max_phase_cutoff
                ]
                self.logger.info(
                    f"Filtered to {len(df_indications)} indications with max_phase >= {self.max_phase_cutoff} (from {initial_count})"
                )
            ...
            self.logger.info(
                f"Processing {len(df_indications)} indication records for insertion"
            )
            ...
            try:
                            ...
                            batch_data.append(
                                {
                                    ...
                                    and pd.notna(row["indication"])
                                    else None,
                                    "max_phase_for_ind": float(row["max_phase_for_ind"])
                                    if pd.notna(row.get("max_phase_for_ind"))
                                    else None,
                                    "mesh_id": str(row["mesh_id"])
                                    if "mesh_id" in row and pd.notna(row["mesh_id"])
                                    else None,
                                    "efo_id": str(row["efo_id"])
                                    if "efo_id" in row and pd.notna(row["efo_id"])
                                    else None,
                                }
                            )
                ...
                self._safe_commit()
                self.logger.info(
                    f"Processed drug_indications: inserted {inserted_count} indications"
                )
    ...
    def _process_drug_publications(self, extracted_dir: Path) -> None:
        ...
        try:
            ...
            if existing_count > 0 and not self.force_download:
                self.logger.info(
                    f"Found {existing_count} existing publications. Use force_download=True to reimport."
                )
                return
            ...
            if not docs_path.exists():
                self.logger.warning(
                    "docs.csv not found, skipping publication processing"
                )
                return
            ...
            self.logger.info(
                f"Processing {len(df_docs)} publication records for insertion"
            )
            ...
            try:
                ...
                self._safe_commit()
                self.logger.info(
                    f"Processed drug_publications: inserted {inserted_count} publications"
                )
    ...
    def import_chembl_to_tables(self, extracted_dir: Path) -> None:
        ...
        try:
            ...
            if count is not None and count[0] > 0 and not self.force_download:
                self.logger.info(
                    f"ChEMBL data already imported ({count[0]} drugs). Use force_download=True to reimport."
                )
                return
            ...
            if self.db_manager.conn is not None and not getattr(
                self.db_manager.conn, "closed", True
            ):
    ...
    def _update_drug_batch(
        self, updates: List[Tuple[str, List[str], str, str]]
    ) -> None:
        ...
        try:
            ...
            try:
                while True:
                    ...
                    if not self.ensure_connection() or not self.db_manager.cursor:
                        raise DatabaseError(
                            "Database connection lost during score calculation"
                        )
                    ...
                    with self.get_db_transaction() as transaction:
                        ...
                        pathway_weight = float(
                            self.config.get("drug_pathway_weight", 1.0)
                        )
                        go_weight = (
                            pathway_weight * 0.5
                        )  # GO terms weighted at 50% of pathway weight
            ...
            try:
                if self.ensure_connection() and self.db_manager.cursor:
                    ...
                    if self.db_manager.conn and not getattr(
                        self.db_manager.conn, "closed", True
                    ):
                        self.db_manager.conn.commit()
            except Exception as e:
    ...
    def _verify_integration_results(self) -> None:
        ...
        if not self.ensure_connection():
            self.logger.warning(
                "Cannot verify results - database connection unavailable"
            )
            return
        ...
        try:
            ...
            if not self.db_manager.cursor:
                self.logger.warning(
                    "Cannot verify results - no database cursor available"
                )
                return
    ...
    def _safe_execute(self, query: str, params: Any = None) -> Optional[Any]:
        ...
        if not self.ensure_connection() or not self.db_manager.cursor:
            self.logger.warning(
                "Cannot execute query - database connection unavailable"
            )
            return None
    ...
    def _safe_fetch_count(
        ...
        if not self.ensure_connection() or not self.db_manager.cursor:
            self.logger.warning(
                f"Cannot fetch count from {table_name} - database connection unavailable"
            )
            return 0
    ...
    def get_clinical_phase_data(self, gene_symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            target_response = self.query_chembl_api(
                "target", {"target_synonym": gene_symbol, "format": "json", "limit": 20}
            )
            ...
            for target in target_response.get("targets", []):
                ...
                activity_response = self.query_chembl_api(
                    "activity",
                    {
                ...
                for activity in activity_response.get("activities", []):
                    ...
                    molecule_response = self.query_chembl_api(
                        "molecule",
                        {"molecule_chembl_id": molecule_chembl_id, "format": "json"},
                    )
                    ...
                    for molecule in molecule_response.get("molecules", []):
                        ...
                        if max_phase is not None and max_phase >= self.max_phase_cutoff:
                            ...
                            if self.include_mechanisms:
                                moa_response = self.query_chembl_api(
                                    "mechanism",
                                    {
                                        "molecule_chembl_id": molecule_chembl_id,
                                        "format": "json",
                                ...
                                for moa in moa_response.get("mechanisms", []):
                                    mechanisms.append(
                                        {
                                            "mechanism_of_action": moa.get(
                                                "mechanism_of_action", ""
                                            ),
                                            "action_type": moa.get("action_type", ""),
                                            "target_chembl_id": moa.get(
                                                "target_chembl_id", ""
                                            ),
                                        }
                                    )
            ...
            self.logger.debug(
                f"Found {len(clinical_drugs)} clinical drug records for {gene_symbol}"
            )
            return clinical_drugs

        except Exception as e:
            self.logger.warning(
                f"Failed to get clinical phase data for {gene_symbol}: {e}"
            )
            return []
    ...
    def enhance_existing_drug_data(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(genes_with_drugs)} genes with existing drug data"
            )
                    ...
                    if clinical_data:
                        ...
                        for drug in clinical_data:
                            ...
                            enhanced_drug_records[drug_id] = {
                                ...
                                "mechanism": drug.get("mechanisms", [{}])[0].get(
                                    "mechanism_of_action", ""
                                ),
    ...
    def _update_enhanced_drug_data(
        self, enhanced_drugs: Dict[str, Dict[str, Any]]
    ) -> None:
        """Update database with enhanced drug data.
        ...
        try:
            self.logger.info(
                f"Updating database with enhanced drug data for {len(enhanced_drugs)} genes"
            )
    ...
    def _populate_publications_from_docs(self) -> None:
        ...
        try:
            schema_name = self.chembl_schema
            self.logger.info(
                "Attempting to populate publications from ChEMBL docs data"
            )
            ...
            for candidate in docs_table_candidates:
                try:
                    self.db_manager.cursor.execute(
                        f"SELECT COUNT(*) FROM {candidate} LIMIT 1"
                    )
            ...
            if not docs_table:
                self.logger.info(
                    "No ChEMBL docs table found, skipping docs-based population"
                )
                return
            ...
            self.logger.info(
                f"Executing flexible query with {len(select_columns)} columns"
            )
            self.db_manager.cursor.execute(query)
            ...
            self.logger.info(
                f"Found {len(docs_data)} publication records in docs table"
            )
            ...
            try:
                for i in range(0, len(docs_data), batch_size):
                    ...
                    for row in batch:
                        ...
                        pmid = (
                            row[column_mapping["pubmed_id"]]
                            if column_mapping["pubmed_id"] < len(row)
                            else None
                        )
                        title = (
                            row[column_mapping["title"]]
                            if column_mapping["title"] < len(row)
                            else None
                        )
                        abstract = (
                            row[column_mapping["abstract"]]
                            if column_mapping["abstract"] < len(row)
                            else None
                        )
                        doi = (
                            row[column_mapping["doi"]]
                            if column_mapping["doi"] < len(row)
                            else None
                        )
                        ...
                        if pmid or doi or title:
                            batch_values.append(
                                (
                                    ...
                                    title,
                                    abstract,
                                    row[column_mapping["year"]]
                                    if column_mapping["year"] < len(row)
                                    else None,
                                    row[column_mapping["journal"]]
                                    if column_mapping["journal"] < len(row)
                                    else None,
                                    row[column_mapping["authors"]]
                                    if column_mapping["authors"] < len(row)
                                    else None,
                                    row[column_mapping["volume"]]
                                    if column_mapping["volume"] < len(row)
                                    else None,
                                    row[column_mapping["issue"]]
                                    if column_mapping["issue"] < len(row)
                                    else None,
                                    row[column_mapping["first_page"]]
                                    if column_mapping["first_page"] < len(row)
                                    else None,
                                    row[column_mapping["last_page"]]
                                    if column_mapping["last_page"] < len(row)
                                    else None,
                                    row[column_mapping["journal_full_title"]]
                                    if column_mapping["journal_full_title"] < len(row)
                                    else None,
                                    row[column_mapping["patent_id"]]
                                    if column_mapping["patent_id"] < len(row)
                                    else None,
                                )
                            )
                ...
                self._safe_commit()
                self.logger.info(
                    f"Successfully populated {inserted_count} publications from docs table"
                )
    ...
    def extract_publication_references(self) -> List[Publication]:
        ...
        try:
            if not self.ensure_connection() or not self.db_manager.cursor:
                self.logger.warning(
                    "Cannot extract publication references - database connection unavailable"
                )
                return publications
            ...
            available_columns = [row[0] for row in self.db_manager.cursor.fetchall()]
            self.logger.info(
                f"Available columns in {schema_name}.drug_publications: {available_columns}"
            )
            ...
            self.logger.info(
                f"Executing flexible publication extraction query with {len(select_columns)} columns"
            )
            self.db_manager.cursor.execute(query)
            ...
            self.logger.info(
                f"Extracting publication references from {len(publication_rows)} ChEMBL publications"
            )
            ...
            for row in publication_rows:
                ...
                chembl_id = (
                    row[column_mapping["chembl_id"]]
                    if column_mapping["chembl_id"] < len(row)
                    else None
                )
                doc_id = (
                    row[column_mapping["doc_id"]]
                    if column_mapping["doc_id"] < len(row)
                    else None
                )
            ...
            self.logger.info(
                f"Extracted {len(publications)} publication references from ChEMBL"
            )
            return publications
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if not self.ensure_schema_version("v0.1.5"):
                raise DatabaseError(
                    f"Database schema version incompatible. Required: v0.1.5"
                )
            ...
            if publications:
                self.logger.info(
                    f"Processing {len(publications)} ChEMBL publication references"
                )
            ...
            if self.include_clinical_phases or self.include_mechanisms:
                self.logger.info(
                    "Enhancing drug data with clinical phases and mechanisms"
                )
            ...
            self.logger.info(
                "ChEMBL drug processing completed successfully with clinical enhancements"
            )
