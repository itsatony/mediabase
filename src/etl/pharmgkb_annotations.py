...
PHARMGKB_CLINICAL_ANNOTATIONS_URL = (
    "https://api.pharmgkb.org/v1/download/file/data/clinicalAnnotations.zip"
)
PHARMGKB_VARIANT_ANNOTATIONS_URL = (
    "https://api.pharmgkb.org/v1/download/file/data/variantAnnotations.zip"
)
PHARMGKB_VIP_SUMMARIES_URL = (
    "https://www.pharmgkb.org/downloads/vipSummaries.tsv"  # Not available via API
)
...
CLINICAL_SIGNIFICANCE_SCORES = {"High": 3, "Moderate": 2, "Low": 1, "Unknown": 0}
...
class PharmGKBAnnotationsProcessor(BaseProcessor):
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        # Data source URLs
        self.clinical_annotations_url = config.get(
            "pharmgkb_clinical_annotations_url", PHARMGKB_CLINICAL_ANNOTATIONS_URL
        )
        self.variant_annotations_url = config.get(
            "pharmgkb_variant_annotations_url", PHARMGKB_VARIANT_ANNOTATIONS_URL
        )
        self.vip_summaries_url = config.get(
            "pharmgkb_vip_summaries_url", PHARMGKB_VIP_SUMMARIES_URL
        )

        # Processing options
        self.skip_scores = config.get("skip_scores", False)
        self.force_download = config.get("force_download", False)
        self.include_variant_annotations = config.get(
            "include_variant_annotations", True
        )
        ...
        self.required_schema_version = (
            "v0.1.9"  # Minimum schema version required for PharmGKB variants
        )
    ...
    def download_pharmgkb_data(
        ...
        try:
            self.logger.info(
                "Downloading PharmGKB datasets from API (no registration required)"
            )
            ...
            self.logger.info(
                f"Downloading clinical annotations from {self.clinical_annotations_url}"
            )
            ...
            if not clinical_file.exists():
                ...
                if tsv_files:
                    ...
                    self.logger.info(
                        f"Using clinical annotations file: {clinical_file.name}"
                    )
                else:
            ...
            if self.include_variant_annotations:
                self.logger.info(
                    f"Downloading variant annotations from {self.variant_annotations_url}"
                )
                ...
                if variant_drug_file.exists():
                    ...
                    self.logger.info(
                        f"Found variant annotations file: {variant_file.name}"
                    )
                else:
                    ...
                    if var_files:
                        ...
                        self.logger.info(
                            f"Using variant annotations file: {variant_file.name}"
                        )
                    else:
                        self.logger.warning(
                            "No variant TSV files found, skipping variant annotations"
                        )
            ...
            if self.include_vip_summaries:
                self.logger.info(
                    "VIP summaries require authentication (skipping automated download)"
                )
            ...
            if self.include_pathways:
                ...
                if pathways_dir_path.exists() and pathways_dir_path.is_dir():
                    ...
                    if pathway_files:
                        ...
                        self.logger.info(
                            f"Found {len(pathway_files)} PharmGKB pathway files"
                        )
                    else:
                        self.logger.info(
                            "Pathway data not available via API (requires manual download)"
                        )
                else:
                    self.logger.info(
                        "Pathway data not available via API (requires manual download)"
            ...
            self.logger.info(
                "PharmGKB data download and extraction completed successfully"
            )
    ...
    def process_clinical_annotations(
        self, clinical_file: Path
        ...
        try:
            ...
            if len(df) == 0 or "Gene" not in df.columns:
                raise ProcessingError(
                    "PharmGKB file appears empty or has unexpected format"
                )

            self.logger.info(
                f"Loaded {len(df):,} clinical annotation records from PharmGKB"
            )
            ...
            try:
                for _, row in df.iterrows():
                    ...
                    annotation_id = (
                        str(row.get("Clinical Annotation ID", ""))
                        if pd.notna(row.get("Clinical Annotation ID"))
                        else ""
                    )
                    gene_field = (
                        row.get("Gene", "") if pd.notna(row.get("Gene")) else ""
                    )
                    variant = (
                        str(row.get("Variant/Haplotypes", ""))
                        if pd.notna(row.get("Variant/Haplotypes"))
                        else ""
                    )
                    drug = (
                        str(row.get("Drug(s)", ""))
                        if pd.notna(row.get("Drug(s)"))
                        else ""
                    )
                    phenotype_category = (
                        str(row.get("Phenotype Category", ""))
                        if pd.notna(row.get("Phenotype Category"))
                        else ""
                    )
                    phenotypes = (
                        str(row.get("Phenotype(s)", ""))
                        if pd.notna(row.get("Phenotype(s)"))
                        else ""
                    )
                    evidence_level = (
                        str(row.get("Level of Evidence", ""))
                        if pd.notna(row.get("Level of Evidence"))
                        else ""
                    )
                    level_modifiers = (
                        str(row.get("Level Modifiers", ""))
                        if pd.notna(row.get("Level Modifiers"))
                        else ""
                    )
                    score = row.get("Score", 0) if pd.notna(row.get("Score")) else 0
                    pmid_count = (
                        row.get("PMID Count", 0)
                        if pd.notna(row.get("PMID Count"))
                        else 0
                    )
                    evidence_count = (
                        row.get("Evidence Count", 0)
                        if pd.notna(row.get("Evidence Count"))
                        else 0
                    )
                    url = str(row.get("URL", "")) if pd.notna(row.get("URL")) else ""
                    specialty_population = (
                        str(row.get("Specialty Population", ""))
                        if pd.notna(row.get("Specialty Population"))
                        else ""
                    )
                    ...
                    annotation_record = {
                        ...
                        "pmid_count": int(pmid_count)
                        if isinstance(pmid_count, (int, float))
                        else 0,
            ...
            self.logger.info(
                f"  - Total annotations: {processing_stats['total_annotations']:,}"
            )
            self.logger.info(
                f"  - Annotations with genes: {processing_stats['annotations_with_genes']:,}"
            )
            self.logger.info(
                f"  - Unique genes: {len(processing_stats['unique_genes']):,}"
            )

            if processing_stats["scores"]:
                avg_score = sum(processing_stats["scores"]) / len(
                    processing_stats["scores"]
                )
            ...
            for category, count in processing_stats[
            ...
            raise ProcessingError(
                f"Failed to process PharmGKB clinical annotations: {e}"
            )
    ...
    def process_vip_summaries(self, vip_file: Path) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            with open(vip_file, "r") as f:
                ...
                if first_line.startswith("<!doctype html") or first_line.startswith(
                    "<html"
                ):
                    self.logger.warning(
                        "PharmGKB VIP file returned HTML - creating mock VIP data"
                    )
            ...
            if len(df) == 0 or "Gene Symbol" not in df.columns:
                self.logger.warning(
                    "PharmGKB VIP file appears empty or has unexpected format"
                )
                self.logger.warning("Creating mock VIP data for demonstration purposes")
                return self._create_mock_vip_data()

            self.logger.info(
                f"Loaded {len(df):,} VIP gene summary records from PharmGKB"
            )
            ...
            for _, row in df.iterrows():
                gene_symbol = (
                    str(row.get("Gene Symbol", "")).strip().upper()
                ...
                clinical_annotation = (
                    str(row.get("Clinical Annotation", ""))
                    if pd.notna(row.get("Clinical Annotation"))
                    else ""
                )
                variant_annotation = (
                    str(row.get("Variant Annotation", ""))
                    if pd.notna(row.get("Variant Annotation"))
                    else ""
                )
                vip_summary = (
                    str(row.get("VIP Summary", ""))
                    if pd.notna(row.get("VIP Summary"))
                    else ""
                )
                ...
                if gene_symbol:
                    vip_mapping[gene_symbol] = {
                        "gene_name": gene_name,
                        "clinical_annotation_count": len(clinical_annotation.split(","))
                        if clinical_annotation
                        else 0,
                        "variant_annotation_count": len(variant_annotation.split(","))
    ...
    def process_pharmgkb_pathways(
        self, pathways_dir: Path
        ...
        try:
            ...
            if not pathway_files:
                raise ProcessingError(
                    "No pathway TSV files found in pathways directory"
                )
            ...
            try:
                for pathway_file in pathway_files:
                    ...
                    if len(pathway_parts) != 2:
                        self.logger.warning(
                            f"Unexpected pathway filename format: {filename}"
                        )
                        continue
                    ...
                    try:
                        ...
                        for _, row in df.iterrows():
                            ...
                            reaction_data = {
                                ...
                                "reaction_type": str(row.get("Reaction Type", ""))
                                if pd.notna(row.get("Reaction Type"))
                                else "",
                                "controller_genes": [],
                                "control_type": str(row.get("Control Type", ""))
                                if pd.notna(row.get("Control Type"))
                                else "",
                                "cell_type": str(row.get("Cell Type", ""))
                                if pd.notna(row.get("Cell Type"))
                                else "",
                                "pmids": [],
                                "drugs": [],
                                "diseases": [],
                            ...
                            controller_field = (
                                row.get("Controller", "")
                                if pd.notna(row.get("Controller"))
                                else ""
                            )
                            if controller_field:
                                controller_genes = [
                                    gene.strip()
                                    for gene in str(controller_field).split(",")
                                    if gene.strip()
                                ]
                                reaction_data["controller_genes"] = controller_genes
                                processing_stats["unique_genes"].update(
                                    controller_genes
                                )
                            ...
                            if genes_field:
                                additional_genes = [
                                    gene.strip()
                                    for gene in str(genes_field).split(",")
                                    if gene.strip()
                                ]
                                reaction_data["controller_genes"].extend(
                                    additional_genes
                                )
                                processing_stats["unique_genes"].update(
                                    additional_genes
                            ...
                            reaction_data["controller_genes"] = list(
                                set(reaction_data["controller_genes"])
                            )

                            # Parse PMIDs
                            pmids_field = (
                                row.get("PMIDs", "")
                                if pd.notna(row.get("PMIDs"))
                                else ""
                            )
                            if pmids_field:
                                pmids = [
                                    pmid.strip()
                                    for pmid in str(pmids_field).split(",")
                                    if pmid.strip()
                                ]
                            ...
                            if drugs_field:
                                drugs = [
                                    drug.strip()
                                    for drug in str(drugs_field).split(",")
                                    if drug.strip()
                                ]
                            ...
                            diseases_field = (
                                row.get("Diseases", "")
                                if pd.notna(row.get("Diseases"))
                                else ""
                            )
                            if diseases_field:
                                diseases = [
                                    disease.strip()
                                    for disease in str(diseases_field).split(",")
                                    if disease.strip()
                                ]
                            ...
                            if reaction_data["reaction_type"]:
                                processing_stats["reaction_types"][
                                    reaction_data["reaction_type"]
                                ] += 1
                            if reaction_data["cell_type"]:
                                processing_stats["cell_types"][
                                    reaction_data["cell_type"]
                                ] += 1
                            ...
                            for gene in reaction_data["controller_genes"]:
                                gene_roles[gene]["reactions"].append(
                                    f"{reaction_data['from']} -> {reaction_data['to']}"
                                )
                                ...
                                if reaction_data["control_type"].lower() == "catalysis":
                                    ...
                                    reaction_data["reaction_type"].lower()
                                    == "transport"
                                ):
                        ...
                        pathway_data = {
                            ...
                            "clinical_relevance": {
                                ...
                                "drug_interactions": len(pathway_reactions)
                                > 5,  # Complex pathways more likely to have interactions
                                "genetic_variants": any(
                                    gene
                                    in ["CYP2D6", "CYP3A4", "CYP2C19", "TPMT", "UGT1A1"]
                            ...
                            "statistics": {
                                ...
                                "pmid_count": sum(
                                    len(r["pmids"]) for r in pathway_reactions
                                ),
                                "drug_mentions": sum(
                                    len(r["drugs"]) for r in pathway_reactions
                                ),
                            },
                        }
                        ...
                        self.logger.error(
                            f"Failed to process pathway file {pathway_file}: {e}"
                        )
                        continue
            ...
            self.logger.info(
                f"  - Total pathway files: {processing_stats['total_pathway_files']:,}"
            )
            self.logger.info(
                f"  - Total reactions: {processing_stats['total_reactions']:,}"
            )
            self.logger.info(
                f"  - Unique genes: {len(processing_stats['unique_genes']):,}"
            )
            self.logger.info(
                f"  - Cancer-relevant pathways: {processing_stats['cancer_relevant_pathways']:,}"
            )
    ...
    def process_variant_annotations(
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(df):,} variant annotation records from PharmGKB"
            )
            ...
            try:
                for _, row in df.iterrows():
                    ...
                    variant_id = (
                        str(row.get("Variant Annotation ID", ""))
                        if pd.notna(row.get("Variant Annotation ID"))
                        else ""
                    )
                    variant_identifier = (
                        str(row.get("Variant/Haplotypes", ""))
                    ...
                    if variant_identifier:
                        if variant_identifier.startswith("rs"):
                            ...
                            "del" in variant_identifier.lower()
                            or "ins" in variant_identifier.lower()
                        ):
                            variant_type = "indel"
                    ...
                    variant_record = {
                        ...
                        "direction_of_effect": str(row.get("Direction of effect", ""))
                        if pd.notna(row.get("Direction of effect"))
                        else "",
                        "pmid": pmid,
                        "clinical_sentence": str(row.get("Sentence", ""))
                        if pd.notna(row.get("Sentence"))
                        else "",
                        "specialty_population": str(row.get("Specialty Population", ""))
                        if pd.notna(row.get("Specialty Population"))
                        else "",
                        "pharmacogenomic_score": pharmacogenomic_score,
                        "evidence_level": self._assess_variant_evidence_level(row),
                        "metabolizer_prediction": self._extract_metabolizer_info(
                            variant_identifier, phenotype_category
                        ),
                        "clinical_actionability": self._assess_clinical_actionability(
                            significance, phenotype_category
                        ),
            ...
            self.logger.info(
                f"  - Total records: {processing_stats['total_records']:,}"
            )
            self.logger.info(
                f"  - Processed records: {processing_stats['processed_records']:,}"
            )
            self.logger.info(
                f"  - Genes with variants: {len(processing_stats['genes_with_variants']):,}"
            )
            ...
            raise ProcessingError(
                f"Failed to process PharmGKB variant annotations: {e}"
            )
    ...
    def _assess_variant_evidence_level(self, row: pd.Series) -> str:
        ...
        if (
            ...
            and pmid
            and pmid.strip()
            and any(cat in category for cat in ["efficacy", "toxicity"])
        ):
            return "high"
        # Moderate evidence: clinical significance or strong category with PMID
            ...
            (
                "yes" in significance
                or any(cat in category for cat in ["efficacy", "toxicity"])
            )
            and pmid
            and pmid.strip()
        ):
    ...
    def _extract_metabolizer_info(
        self, variant_identifier: str, phenotype_category: str
    ) -> Dict[str, Any]:
        ...
        if "*" in variant_identifier:
            ...
            if "cyp2d6" in phenotype_category.lower():
                if any(allele in gene_allele for allele in ["*4", "*5", "*6", "*10"]):
                    metabolizer_info.update(
                        {"phenotype": "poor_metabolizer", "confidence": 0.85}
                    )
                elif any(allele in gene_allele for allele in ["*9", "*41"]):
                    metabolizer_info.update(
                        {"phenotype": "intermediate_metabolizer", "confidence": 0.80}
                    )
                elif any(allele in gene_allele for allele in ["*1", "*2"]):
                    metabolizer_info.update(
                        {"phenotype": "extensive_metabolizer", "confidence": 0.75}
                    )
            ...
            # CYP2C19 metabolizer predictions
                ...
                if any(allele in gene_allele for allele in ["*2", "*3"]):
                    ...
                    metabolizer_info.update(
                        {"phenotype": "ultrarapid_metabolizer", "confidence": 0.80}
                    )

        return metabolizer_info

    def _assess_clinical_actionability(
        self, significance: str, phenotype_category: str
    ) -> str:
        ...
        if "yes" in significance and any(
            cat in category for cat in ["dosage", "efficacy", "toxicity"]
        ):
            return "high"
    ...
    def extract_publication_references(
        ...
        variant_mapping: Optional[Dict[str, List[Dict[str, Any]]]] = None,
    ) -> List[Publication]:
                                ...
                                publications.append(
                                    {
                                        ...
                                        "phenotype_category": annotation.get(
                                            "phenotype_category", ""
                                        ),
                                        "evidence_level": annotation.get(
                                            "evidence_level", ""
                                        ),
                            ...
                            publications.append(
                                {
                                    ...
                                    "phenotype_category": variant.get(
                                        "phenotype_category", ""
                                    ),
                                    "significance": variant.get("significance", ""),
                                    "drugs": variant.get("drugs", []),
                                    "clinical_actionability": variant.get(
        ...
        self.logger.info(
            f"Extracted {len(publications)} publication references from PharmGKB data"
        )
        return publications

    def update_transcript_pharmgkb_data(
        self,
        gene_annotation_mapping: Dict[str, List[Dict[str, Any]]],
        ...
        try:
            self.logger.info(
                "Updating transcript records with PharmGKB pharmacogenomic data"
            )
            ...
            all_pharmgkb_genes = (
                annotation_genes.union(vip_genes)
                .union(pathway_genes)
                .union(variant_genes)
            )
            ...
            self.logger.info(
                f"Found {len(matching_genes):,} genes in database that have PharmGKB data"
            )
            self.logger.info(
                f"Database genes: {len(db_genes):,}, PharmGKB genes: {len(all_pharmgkb_genes):,}"
            )

            if not matching_genes:
                self.logger.warning(
                    "No gene overlap found between database and PharmGKB"
                )
                return
                ...
                for gene_symbol in matching_genes:
                    ...
                    if gene_symbol in gene_annotation_mapping:
                        ...
                        max_evidence_score = max(
                            [ann["evidence_score"] for ann in annotations], default=0
                        )
                        avg_score = (
                            sum([ann["score"] for ann in annotations])
                            / total_annotations
                        ...
                        total_evidence = sum(
                            [ann["evidence_count"] for ann in annotations]
                        )
                            ...
                            "evidence_levels": list(
                                set(
                                    [
                                        ...
                                        if ann["evidence_level"]
                                    ]
                                )
                            ),
                            "drugs_mentioned": list(
                                set([ann["drug"] for ann in annotations if ann["drug"]])
                            ),
                            "phenotype_categories": list(
                                set(
                                    [
                                        ann["phenotype_category"]
                                        for ann in annotations
                                        if ann["phenotype_category"]
                                    ]
                                )
                            ),
                            "level_modifiers": list(
                                set(
                                    [
                                        ann["level_modifiers"]
                                        for ann in annotations
                                        if ann["level_modifiers"]
                                    ]
                                )
                            ),
                        }
                    ...
                    if pathway_mapping and gene_symbol in pathway_mapping:
                        ...
                        pathway_stats = {
                            ...
                            "cancer_relevant_pathways": sum(
                                ...
                                if p.get("clinical_relevance", {}).get(
                                    "cancer_relevance", False
                                )
                            ),
                            "enzyme_pathways": sum(
                                1
                                ...
                                if any(
                                    r.get("role") == "enzyme"
                                    for r in p.get("gene_roles", {}).values()
                                )
                            ),
                            "total_reactions": sum(
                                p.get("statistics", {}).get("total_reactions", 0)
                                for p in pathway_mapping[gene_symbol].values()
                            ),
                            "total_pmids": sum(
                                p.get("statistics", {}).get("pmid_count", 0)
                                for p in pathway_mapping[gene_symbol].values()
                            ),
                        }
                        ...
                        variant_data = {
                            "summary": {
                                ...
                                "high_impact_variants": sum(
                                    1
                                    for v in variants
                                    if v.get("pharmacogenomic_score", 0) >= 70
                                ),
                                "clinical_actionable": sum(
                                    1
                                    for v in variants
                                    if v.get("clinical_actionability") == "high"
                                ),
                                "metabolizer_phenotypes": list(
                                    set(
                                        [
                                            v.get("metabolizer_prediction", {}).get(
                                                "phenotype", "unknown"
                                            )
                                            for v in variants
                                            if v.get("metabolizer_prediction", {}).get(
                                                "phenotype"
                                            )
                                            != "unknown"
                                        ]
                                ...
                                "drug_categories": list(
                                    set(
                                        [
                                            v.get("phenotype_category", "")
                                            for v in variants
                                            if v.get("phenotype_category")
                                        ]
                                    )
                                ),
                                "evidence_levels": list(
                                    set(
                                        [
                                            v.get("evidence_level", "")
                                            for v in variants
                                            if v.get("evidence_level")
                                        ]
                                    )
                                ),
                                "max_pharmacogenomic_score": max(
                                    [
                                        v.get("pharmacogenomic_score", 0)
                                        for v in variants
                                    ],
                                    default=0,
                                ),
                                "avg_pharmacogenomic_score": round(
                                    sum(
                                        [
                                            v.get("pharmacogenomic_score", 0)
                                            for v in variants
                                ...
                                "pmid_count": sum(
                                    1
                                    for v in variants
                                    if v.get("pmid") and v.get("pmid").strip()
                                ),
                                "drugs_affected": list(
                                    set(
                                        [
                                ...
                                "specialty_populations": list(
                                    set(
                                        [
                                            v.get("specialty_population", "")
                                            for v in variants
                                            if v.get("specialty_population")
                                            and v.get("specialty_population").strip()
                                        ]
                                    )
                                ),
                            },
                            "high_impact_variants": [
                                {
                                    ...
                                    "drugs": v.get("drugs", []),
                                    "phenotype_category": v.get(
                                        "phenotype_category", ""
                                    ),
                                    "significance": v.get("significance", ""),
                                    "direction_of_effect": v.get(
                                        "direction_of_effect", ""
                                    ),
                                    "pharmacogenomic_score": v.get(
                                        "pharmacogenomic_score", 0
                                    ),
                                    "clinical_actionability": v.get(
                                        "clinical_actionability", "research_only"
                                    ),
                                    "evidence_level": v.get(
                                        "evidence_level", "insufficient"
                                    ),
                                    "metabolizer_prediction": v.get(
                                        "metabolizer_prediction", {}
                                    ),
                                    "pmid": v.get("pmid", ""),
                                    "clinical_sentence": v.get("clinical_sentence", "")[
                                        :200
                                    ]
                                    + "..."
                                    if len(v.get("clinical_sentence", "")) > 200
                                    else v.get(
                                        "clinical_sentence", ""
                                    ),  # Truncate long sentences
                                    "specialty_population": v.get(
                                        "specialty_population", ""
                                    ),
                                }
                                for v in variants
                                if (
                                    v.get("pharmacogenomic_score", 0) >= 70
                                    or v.get("clinical_actionability") == "high"
                                    or v.get("significance") == "yes"
                                )
                            ],
                            "cyp450_variants": [
                                {
                                ...
                                for v in variants
                                if any(
                                    cyp in v.get("phenotype_category", "").lower()
                                    for cyp in ["cyp", "metabolism"]
                                )
                            ],
                            "cancer_relevant_variants": [
                                {
                                    ...
                                    + "..."
                                    if len(v.get("clinical_sentence", "")) > 150
                                    else v.get("clinical_sentence", ""),
                                }
                                for v in variants
                                if any(
                                    drug
                    ...
                    if variant_mapping and gene_symbol in variant_mapping:
                        ...
                        update_data.append(
                            (json.dumps(pharmgkb_data), "{}", gene_symbol)
                        )
                    progress_bar.update(1)
            ...
            self.logger.info(
                f"Updating {len(update_data):,} transcript records with PharmGKB data"
            )
            ...
            try:
                ...
                """,
                    [
                        (json_data, json_data, json_data, variant_data, gene_symbol)
                        for json_data, variant_data, gene_symbol in update_data
                    ],
                )
            ...
            self.logger.info(
                f"Successfully updated {updated_count:,} records with PharmGKB clinical data"
            )
            self.logger.info(
                f"Successfully updated {pathway_count:,} records with PharmGKB pathway data"
            )
            self.logger.info(
                f"Successfully updated {variant_count:,} records with PharmGKB variant data"
            )
    ...
    def calculate_pharmgkb_scores(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Calculating scores for {len(genes_with_pharmgkb):,} genes with PharmGKB data"
            )
            ...
            try:
                for gene_symbol, pharmgkb_data in genes_with_pharmgkb:
                    ...
                    if "annotation_summary" in pharmgkb_data:
                        ...
                        overall_score = (
                            evidence_score * 2.0
                            + avg_score * 1.0  # Evidence level weight
                            + min(annotation_count * 0.3, 3.0)  # PharmGKB score weight
                            + min(  # Annotation count bonus (capped)
                                pmid_count * 0.2, 2.0
                            )  # Publication support bonus (capped)
                        )
                    ...
                    if "vip_info" in pharmgkb_data:
                        ...
                        pharmgkb_scores["overall_pharmgkb_score"] = (
                            pharmgkb_scores.get("overall_pharmgkb_score", 0) + 1.0
                        )
            ...
            if score_updates:
                self.logger.info(
                    f"Updating PharmGKB scores for {len(score_updates):,} genes"
                )
    ...
    def generate_pharmgkb_summary(self) -> None:
        ...
        try:
            ...
            coverage = (
                (genes_with_pharmgkb / total_genes * 100) if total_genes > 0 else 0
            )
            pathway_coverage = (
                (genes_with_pathways / total_genes * 100) if total_genes > 0 else 0
            )
            vip_coverage = (vip_genes / total_genes * 100) if total_genes > 0 else 0

            table.add_row("Total Genes", f"{total_genes:,}", "100.0%")
            table.add_row(
                "With PharmGKB Data", f"{genes_with_pharmgkb:,}", f"{coverage:.1f}%"
            )
            table.add_row(
                "With Pathway Data",
                f"{genes_with_pathways:,}",
                f"{pathway_coverage:.1f}%",
            )
            ...
            self.logger.info(
                f"  - Genes with PharmGKB data: {genes_with_pharmgkb:,} ({coverage:.1f}%)"
            )
            self.logger.info(
                f"  - Genes with pathway data: {genes_with_pathways:,} ({pathway_coverage:.1f}%)"
            )
    ...
    def run(self) -> None:
        ...
        try:
            ...
            publications = self.extract_publication_references(
                gene_annotation_mapping, variant_mapping
            )
            ...
            self.update_transcript_pharmgkb_data(
                gene_annotation_mapping, vip_mapping, pathway_mapping, variant_mapping
            )
