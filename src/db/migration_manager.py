...
class MigrationManager:
    ...
    def migrate_if_needed(self) -> bool:
        ...
        if (
            ...
            or self._compare_versions(current_version, "0.1.5") < 0
        ):
            logger.error(
                f"Database version {current_version} is not supported. Please upgrade to v0.1.5 first."
            )
            return False

        # Migration logic for versions >= 0.1.5 would go here if needed
        logger.info(
            f"Database version {current_version} is supported. No migration needed."
        )
        return True
