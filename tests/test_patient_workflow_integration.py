...
class TestPatientWorkflowIntegration:
    ...
    def test_complete_deseq2_workflow_simulation(self, tmp_path: Path):
        ...
        with patch("scripts.create_patient_copy.get_db_manager") as mock_db_manager:
            ...
            for transcript_id, expected_value in expected_conversions.items():
                ...
                ), f"Conversion error for {transcript_id}: {actual_value} vs {expected_value}"
    ...
    def test_standard_format_vs_deseq2_comparison(self, tmp_path: Path):
        ...
        standard_creator = PatientDatabaseCreator(
            "STANDARD_PATIENT", standard_file, db_config
        )
        ...
        deseq2_creator = PatientDatabaseCreator(
            "DESEQ2_PATIENT", deseq2_file, db_config
        )
    ...
    def test_clinical_workflow_simulation(self, tmp_path: Path):
        ...
        clinical_creator = PatientDatabaseCreator(
            "CLINICAL_001", clinical_file, db_config
        )
        ...
        clinical_mappings = [
            ...
            ("ATM", "ENST00000278616"),
            ("CHEK2", "ENST00000382580"),
            ("EGFR", "ENST00000275493"),
            ("HER2", "ENST00000269571"),
            ("MYC", "ENST00000377970"),
            ("RAS", "ENST00000256078"),
            ("PI3K", "ENST00000263967"),
            ("AKT1", "ENST00000349310"),
            ("ESR1", "ENST00000206249"),
            ("PGR", "ENST00000315146"),
            ("AR", "ENST00000374690"),
            ("CCND1", "ENST00000227507"),
            ("RB1", "ENST00000267163"),
            ("CDK4", "ENST00000257904"),
        ]
        ...
        with patch("scripts.create_patient_copy.get_db_manager") as mock_db_manager:
            ...
            upregulated_count = sum(
                1 for fold in clinical_creator.transcript_updates.values() if fold > 2.0
            )
            downregulated_count = sum(
                1 for fold in clinical_creator.transcript_updates.values() if fold < 0.5
            )

            assert upregulated_count > 0, "Should have some highly upregulated genes"
            assert (
                downregulated_count > 0
            ), "Should have some highly downregulated genes"
