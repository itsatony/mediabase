...
class PathwayProcessor(BaseProcessor):
    ...
    def process_pathways(self) -> Dict[str, Set[str]]:
        ...
        try:
            ...
            avg_pathways = sum(
                len(pathways) for pathways in gene_to_pathways.values()
            ) / max(1, total_genes)
            ...
            self.logger.info(
                "Extracting pathway publication references from Reactome API"
            )
            ...
            if pathway_publications:
                ...
                self.logger.info(
                    f"Saved publication data for {len(pathway_publications)} pathways"
                )
    ...
    def _extract_pathway_publications(
        self, evidence: str, pathway_id: str
    ) -> List[Publication]:
    ...
    def _fetch_reactome_publications(
        self, pathway_to_genes: Dict[str, Set[str]]
        ...
        self.logger.info(
            f"Fetching publications for {len(pathway_ids)} pathways from Reactome API"
        )
        ...
        for pathway_id in tqdm(pathway_ids, desc="Fetching pathway publications"):
            try:
                ...
                if response.status_code == 200:
                    ...
                    self.logger.warning(
                        f"Failed to fetch publications for pathway {pathway_id}: HTTP {response.status_code}"
                    )

            except requests.exceptions.Timeout:
                self.logger.warning(
                    f"Timeout fetching publications for pathway {pathway_id}"
                )
                continue
            except Exception as e:
                self.logger.warning(
                    f"Error fetching publications for pathway {pathway_id}: {e}"
                )
                continue
    ...
    def _save_pathway_publications(
        self, pathway_publications: Dict[str, List[Publication]]
    ) -> None:
    ...
    def _get_ncbi_mapping(self) -> Dict[str, str]:
        ...
        try:
            ...
            self.logger.info(
                f"Found {len(rows)} NCBI ID mappings in gene_cross_references"
            )
            ...
            if not mapping:
                ...
                self.logger.info(
                    f"Using {len(mapping)} gene symbols for direct mapping"
                )
            else:
                self.logger.info(
    ...
    def enrich_transcripts(self, gene_to_pathways: Dict[str, Set[str]]) -> None:
        ...
        try:
            ...
            for gene_symbol, pathways in tqdm(
                gene_to_pathways.items(), desc="Preparing pathway updates"
            ):
                ...
                # Add update to batch
                updates.append((list(pathways), json.dumps(gene_publications), db_gene))
        ...
        try:
            ...
            for pathways_list, publications_json, gene_symbol in updates:
                ...
                for pathway_string in pathways_list:
                    ...
                    match = re.search(
                        r"^(.+)\s+\[Reactome:(R-[A-Z]+-\d+)\]$", pathway_string
                    )
                    if match:
                        ...
                        pathway_inserts.append(
                            (gene_symbol, pathway_id, pathway_name, "Reactome")
                        )
            ...
            if pathway_inserts:
                ...
                """
                    % ",".join(
                        [
                            self.db_manager.cursor.mogrify("(%s,%s,%s,%s)", row).decode(
                                "utf-8"
                            )
                ...
                self.logger.debug(
                    f"Inserted {len(pathway_inserts)} pathway mappings from batch of {len(updates)} genes"
                )
    ...
    def extract_pathway_references(
    ...
    def integrate_pathways(self, pathway_data: Dict[str, List[Dict[str, Any]]]) -> None:
        ...
        try:
            ...
            with self.get_db_transaction() as transaction:
                ...
                self.logger.debug(
                    f"Legacy table updates skipped (using normalized schema)"
                )
            ...
            self.logger.info(
                f"Successfully integrated pathway data for {processed} genes"
            )
    ...
    def _update_pathway_batch(
        self, updates: List[Tuple[str, List[str], List[str], str, str]]
    ) -> None:
