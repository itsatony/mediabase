...
class TestPatientDatabaseCreator:
    ...
    @pytest.fixture
    def creator(
        self, csv_file: Path, db_config: Dict[str, Any]
    ) -> PatientDatabaseCreator:
    ...
    def test_validate_csv_file_missing_columns(
        self, tmp_path: Path, db_config: Dict[str, Any], invalid_csv_data: pd.DataFrame
    ):
        ...
        with patch("rich.prompt.Prompt.ask", side_effect=["wrong_id", "wrong_fold"]):
            with patch("pandas.to_numeric", side_effect=ValueError("Invalid values")):
                with pytest.raises(
                    CSVValidationError, match="Failed to validate fold-change values"
                ):
                    creator.validate_csv_file()
    ...
    def test_find_column_mapping_case_insensitive(
        self, creator: PatientDatabaseCreator
    ):
    ...
    def test_validate_mapped_columns_success(
        self, creator: PatientDatabaseCreator, valid_csv_data: pd.DataFrame
    ):
        """Test successful column validation."""
        creator.csv_data = valid_csv_data
        creator.column_mapping = {
            "transcript_id": "transcript_id",
            "cancer_fold": "cancer_fold",
        }
    ...
    def test_validate_mapped_columns_invalid_numbers(
        self, creator: PatientDatabaseCreator
    ):
    ...
    def test_create_target_database_new(
        self, mock_connect: Mock, creator: PatientDatabaseCreator
    ):
        ...
        mock_cursor.execute.assert_any_call(
            'CREATE DATABASE "mediabase_patient_TEST123"'
        )

    @patch("psycopg2.connect")
    @patch("rich.prompt.Confirm.ask", return_value=True)
    def test_create_target_database_overwrite(
        self, mock_confirm: Mock, mock_connect: Mock, creator: PatientDatabaseCreator
    ):
    ...
    def test_create_target_database_cancelled(
        self, mock_confirm: Mock, mock_connect: Mock, creator: PatientDatabaseCreator
    ):
        ...
        with pytest.raises(
            DatabaseCopyError, match="Database creation cancelled by user"
        ):
            creator._create_target_database(Mock())
    ...
    @patch("os.remove")
    def test_copy_database_content(
        self,
        mock_remove: Mock,
        mock_exists: Mock,
        mock_system: Mock,
        creator: PatientDatabaseCreator,
    ):
...
class TestCSVValidationEdgeCases:
    ...
    def test_mixed_case_columns(self, tmp_path: Path, db_config: Dict[str, Any]):
        ...
        data = pd.DataFrame(
            {"Transcript_ID": ["ENST00000123456"], "Cancer_Fold": [2.5]}
        )
    ...
    def test_scientific_notation_fold_values(
    ...
    def test_negative_fold_values(self, tmp_path: Path, db_config: Dict[str, Any]):
        ...
        data = pd.DataFrame(
            {"transcript_id": ["ENST00000123456"], "cancer_fold": [-1.5]}
        )
