...
class PatientDatabaseManager:
                ...
                for db_name, size, collate, conn_limit, active_conns in databases:
                    ...
                    if show_details:
                        ...
                        try:
                            ...
                            self.logger.warning(
                                f"Could not get details for {db_name}: {e}"
                            )
    ...
    def _get_database_details(self, db_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            with patient_db.transaction() as cursor:
                ...
                try:
                    cursor.execute(
                        "SELECT version_name FROM schema_version ORDER BY applied_at DESC LIMIT 1"
                    )
    ...
    def display_patient_databases(self, patient_dbs: List[Dict[str, Any]]) -> None:
        ...
        if total_modified > 0:
            self.console.print(
                f"Total modified transcripts across all patients: {total_modified:,}"
            )
    ...
    def display_database_info(self, patient_id: str) -> None:
        ...
        if db_info.get("creation_time"):
            ...
            info_text.append(
                f"{db_info['creation_time'].strftime('%Y-%m-%d %H:%M:%S')}\n"
            )

        panel = Panel(
            info_text, title=f"Patient Database: {patient_id}", border_style="blue"
        )
        self.console.print(panel)

        # Show sample of modified transcripts
        if (
            isinstance(db_info["modified_transcripts"], int)
            and db_info["modified_transcripts"] > 0
        ):
    ...
    def _display_sample_transcripts(self, db_name: str, limit: int = 10) -> None:
        ...
        try:
            ...
            with patient_db.transaction() as cursor:
                ...
                if samples:
                    table = Table(
                        title=f"Sample Modified Transcripts (Top {len(samples)})"
                    )
                    ...
                    for transcript_id, gene_symbol, fold_change in samples:
                        table.add_row(
                            transcript_id, gene_symbol or "N/A", f"{fold_change:.4f}"
                        )
    ...
    def delete_patient_database(self, patient_id: str, force: bool = False) -> bool:
        ...
        try:
            ...
            self.console.print(
                f"[green]✓ Patient database '{patient_id}' deleted successfully.[/green]"
            )
            return True
    ...
    def cleanup_old_databases(self, days_old: int, dry_run: bool = False) -> int:
        ...
        if not old_databases:
            self.console.print(
                f"[green]No patient databases older than {days_old} days found.[/green]"
            )
            return 0
...
Examples:
  ...
  %(prog)s --cleanup --older-than 7 --dry-run  # Preview cleanup without deleting
    ...
    action_group.add_argument(
        "--list", action="store_true", help="List all patient databases"
    )
    ...
    action_group.add_argument(
        "--delete", metavar="PATIENT_ID", help="Delete specific patient database"
    )
    ...
    try:
        ...
        if args.list:
            console.print(
                "[bold blue]MEDIABASE Patient Database Management[/bold blue]"
            )
            ...
            deleted_count = manager.cleanup_old_databases(
                args.older_than, dry_run=args.dry_run
            )
            if not args.dry_run:
                console.print(
                    f"[green]Cleanup completed: {deleted_count} databases deleted.[/green]"
                )
