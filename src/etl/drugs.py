...
class DrugProcessor(BaseProcessor):
    ...
    def process_drug_targets(self, drug_data_path: Path) -> pd.DataFrame:
        ...
        try:
            ...
            if "SWISSPROT" in df.columns:
                ...
                "swissprot" not in df.columns and "SWISSPROT" in column_mapping.values()
            ):
            ...
            if result_df.empty:
                raise ValueError(
                    "No valid drug target relationships found after processing"
                )

            self.logger.info(
                f"Successfully processed {len(result_df):,} valid drug-target relationships"
            )
            self.logger.info(f"Found {result_df['drug_id'].nunique():,} unique drugs")
            self.logger.info(
                f"Found {result_df['gene_symbol'].nunique():,} unique genes"
            )
    ...
    def _normalize_drug_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        with PandasOperationSafe():
            ...
            df.loc[:, "evidence_score"] = pd.to_numeric(
                df["evidence_score"], errors="coerce"
            )

            # Fill missing values
            df.loc[:, "mechanism"] = df.get("mechanism", pd.Series("unknown")).fillna(
                "unknown"
            )
            df.loc[:, "action_type"] = df.get(
                "action_type", pd.Series("unknown")
            ).fillna("unknown")
            df.loc[:, "evidence_type"] = df.get(
                "evidence_type", pd.Series("experimental")
            ).fillna("experimental")
            df.loc[:, "evidence_score"] = df.get(
                "evidence_score", pd.Series(1.0)
            ).fillna(1.0)
    ...
    def _process_drug_rows(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for _, row in tqdm(
            df.iterrows(), total=len(df), desc="Processing drug targets"
        ):
            try:
                processed_row = {
                    ...
                    "reference_ids": str(row.get("references", "")).split("|")
                ...
                if "references" in row and row["references"]:
                    ...
                    if len(processed_data) % 500 == 0:
                        self.logger.debug(
                            f"Row {len(processed_data)}: Reference text '{row['references']}' -> {len(references)} publications"
                        )
    ...
    def integrate_drugs(self, drug_targets: pd.DataFrame) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Building normalized gene symbol map for {len(db_genes)} database genes"
            )
            ...
            self.logger.info(
                f"Loaded {len(uniprot_to_gene)} UniProt ID mappings from database"
            )
            ...
            if "swissprot" in drug_targets.columns:
                ...
                self.logger.info(
                    f"Extracted {len(swissprot_map)} SwissProt IDs from drug targets"
                )
            ...
            if match_stats["unmatched_genes"] > 0:
                unmatched_sample = [
                    g
                    for g in drug_genes
                    if g not in enhanced_matches or not enhanced_matches[g]
                ][:10]
            ...
            if self.db_manager.cursor:
                ...
                self.logger.info(
                    f"Genes with drug interactions before processing: {result[0] if result else 0}"
                )

            # Debug sample of reference_ids in drug_targets
            has_refs = drug_targets["reference_ids"].apply(
                lambda x: bool(x) if isinstance(x, list) else False
            )
            ref_count = has_refs.sum()
            self.logger.info(
                f"Drug targets with non-empty reference_ids: {ref_count} out of {len(drug_targets)}"
            )
            ...
            if ref_count > 0:
                ...
                for idx, row in sample_refs.iterrows():
                    self.logger.info(
                        f"Sample reference for {row['gene_symbol']}: {row['reference_ids']}"
                    )
            ...
            for gene, group in drug_targets.groupby("gene_symbol"):
                ...
                if not db_gene:
                    ...
                    if unmatched_count <= 10:  # Only log the first few unmatched genes
                        self.logger.debug(
                            f"No matching gene found for drug target gene: {gene}"
                        )
                ...
                if debug_this_gene:
                    self.logger.info(
                        f"===== Debug: Processing gene {db_gene} (match #{matched_count}) ====="
                    )
                ...
                for _, row in group.iterrows():
                    ...
                    if row.get("reference_ids"):
                        for ref_id in row.get("reference_ids", []):
                            if ref_id and isinstance(ref_id, str):
                                ...
                                if debug_this_gene:
                                    self.logger.info(
                                        f"  Processing reference ID: '{ref_id}'"
                                    )
                                ...
                                if not ref_id.isdigit():
                                    if debug_this_gene:
                                        self.logger.info(
                                            f"  Skipping non-PMID reference: '{ref_id}'"
                                        )
                                    continue
                    ...
                    if act_source_url or moa_source_url:
                        url_pmids = extract_pmids_from_urls(
                            act_source_url, moa_source_url
                        )
            ...
            with self.get_db_transaction() as transaction:
                self.logger.debug(
                    "Legacy table updates skipped (using normalized schema)"
                )
            ...
            if self.db_manager.cursor:
                ...
                result = self.db_manager.cursor.fetchone()
                self.logger.info(
                    f"Genes with drug interactions after processing: {result[0] if result else 0}"
                )
            ...
            try:
                if self.db_manager.cursor and not self.db_manager.cursor.closed:
                    self.db_manager.cursor.execute(
                        "DROP TABLE IF EXISTS temp_drug_data"
                    )
                if self.db_manager.conn and not self.db_manager.conn.closed:
                    self.db_manager.conn.commit()
            except Exception as e:
        ...
        try:
            ...
            with self.get_db_transaction() as transaction:
                ...
                try:
                    ...
                    self.logger.warning(
                        f"Error in batch drug data insert, processing individually: {e}"
                    )
                    ...
                    for (
                        ...
                        if gene_symbol in gene_data:
                            ...
                            merged_data = {
                                **existing_data,
                                **drug_data,
                            }  # Merge drug dictionaries
                            merged_refs = (
                                existing_refs + drug_refs
                            )  # Combine references
                ...
                self.logger.debug(
                    "Legacy table update skipped (using normalized schema)"
                )
        except Exception as e:
    ...
    def calculate_drug_scores(self) -> None:
        ...
        """
        self.logger.info(
            "Drug scoring skipped (algorithm removed - see gene_drug_interactions table for drug data)"
        )
        return
    ...
    def extract_publication_references(self, drug_references: str) -> List[Publication]:
        ...
        for pmid in pmids:
            publication = PublicationsProcessor.create_publication_reference(
                pmid=pmid, evidence_type="DrugCentral", source_db="DrugCentral"
            )
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "Integrating drug data with gene_drug_interactions table..."
            )
    ...
    def _verify_integration_results(self) -> None:
        ...
        try:
            ...
            if stats:
                ...
                if stats[1] == 0:
                    self.logger.warning(
                        "No drug interactions were found! Checking gene_drug_interactions table..."
                    )
                    ...
                    self.logger.info(
                        f"Total rows in gene_drug_interactions: {count[0] if count else 0}"
                    )
                    ...
                    if count and count[0] > 0:
                        ...
                        for record in sample_records:
                            self.logger.info(
                                f"Sample: {record[0]} - {record[1]} ({record[2]})"
                            )
