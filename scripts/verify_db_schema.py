...
def main() -> int:
    ...
    try:
        ...
        if current_version and current_version < "v0.1.5":
            console.print(
                f"[yellow]Warning:[/yellow] Current schema version {current_version} is below minimum supported version v0.1.5"
            )
            console.print(
                "[yellow]You should reset the database to get a fresh v0.1.5 schema[/yellow]"
            )
        ...
        if db_manager.validate_schema():
            ...
            console.print(
                "[red]Schema validation failed - database may need reset[/red]"
            )
            return 1
