...
def list_created_databases():
    ...
    try:
        cmd = [
            'psql', '-h', 'localhost', '-p', '5435', '-U', 'mbase_user', '-d', 'postgres',
            '-c', "SELECT datname FROM pg_database WHERE datname LIKE 'mediabase_patient_DEMO_%' ORDER BY datname;"
        ]
...
def main():
    ...
    console.print("This will create 6 comprehensive patient databases with realistic expression data.\n")
