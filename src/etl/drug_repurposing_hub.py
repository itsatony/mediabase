...
class DrugRepurposingHubProcessor(BaseProcessor):
    ...
    def download_repurposing_hub_data(self) -> Path:
        ...
        try:
            ...
            hub_file = self.download_file(
                url=self.hub_url,
                file_path=self.hub_dir / "repurposing_drugs.txt"
            )
            return hub_file
    ...
    def parse_target_string(self, target_string: str) -> List[str]:
        ...
        targets = [target.strip().upper() for target in str(target_string).split('|') if target.strip()]
        return targets

    def process_repurposing_hub_data(self, hub_file: Path) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(df):,} drug records from Drug Repurposing Hub")
            ...
            try:
                for _, row in df.iterrows():
                    drug_name = str(row['pert_iname']) if pd.notna(row['pert_iname']) else 'Unknown'
                    clinical_phase = str(row['clinical_phase']) if pd.notna(row['clinical_phase']) else 'Unknown'
                    moa = str(row['moa']) if pd.notna(row['moa']) else 'Unknown mechanism'
                    target_string = row['target'] if pd.notna(row['target']) else ''
                    disease_area = str(row['disease_area']) if pd.notna(row['disease_area']) else 'Unknown'
                    indication = str(row['indication']) if pd.notna(row['indication']) else 'Unknown'
            ...
            self.logger.info(f"  - Drugs with targets: {processing_stats['drugs_with_targets']:,}")
            self.logger.info(f"  - Unique target genes: {len(processing_stats['unique_targets']):,}")
    ...
    def update_transcript_drug_data(self, gene_drug_mapping: Dict[str, List[Dict[str, Any]]]) -> None:
        ...
        try:
            self.logger.info("Updating transcript records with Drug Repurposing Hub data")
            ...
            self.logger.info(f"Found {len(matching_genes):,} genes in database that have drug repurposing data")
            self.logger.info(f"Database genes: {len(db_genes):,}, Drug targets: {len(target_genes):,}")
            
            if not matching_genes:
                self.logger.warning("No gene overlap found between database and Drug Repurposing Hub")
                return
            ...
            self.logger.info(f"Updating {len(update_data):,} transcript records with drug repurposing data")
            ...
            self.logger.info(f"Successfully updated {updated_count:,} records with drug repurposing data")
    ...
    def calculate_repurposing_scores(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"Calculating scores for {len(genes_with_drugs):,} genes with drug repurposing data")
            ...
            try:
                for gene_symbol, drugs_json in genes_with_drugs:
                    ...
                    if total_drugs > 0:
                        ...
                        overall_score = max_clinical_score + (total_drugs * 0.1)  # Bonus for multiple options
                        repurposing_scores['overall_repurposing_score'] = round(overall_score, 2)
                        repurposing_scores['total_repurposing_drugs'] = total_drugs
                        repurposing_scores['max_clinical_phase_score'] = max_clinical_score
                        
                        score_updates.append((json.dumps(repurposing_scores), gene_symbol))
    ...
    def generate_repurposing_summary(self) -> None:
        ...
        try:
            ...
            coverage = (genes_with_repurposing / total_genes * 100) if total_genes > 0 else 0
            
            table.add_row("Total Genes", f"{total_genes:,}", "100.0%")
            table.add_row("With Repurposing Data", f"{genes_with_repurposing:,}", f"{coverage:.1f}%")
            ...
            self.logger.info(f"  - Genes with repurposing data: {genes_with_repurposing:,} ({coverage:.1f}%)")
