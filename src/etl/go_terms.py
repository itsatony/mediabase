...
class GOTermProcessor(BaseProcessor):
    ...
    def download_obo(self) -> Path:
        ...
        try:
            ...
            obo_file = self.download_file(
                url=self.go_obo_url, file_path=self.go_dir / "go.obo"
            )
            return obo_file
    ...
    def download_goa(self) -> Path:
        ...
        try:
            ...
            goa_file = self.download_file(
                url=self.goa_url, file_path=self.go_dir / "goa_human.gaf.gz"
            )
            return goa_file
        except Exception as e:
    ...
    def get_ancestors(self, term_id: str, aspect: Optional[str] = None) -> Set[str]:
        ...
        while to_visit:
            ...
            for _, parent, data in self.go_graph.out_edges(current, data=True):
                ...
                if (
                    not data
                    or "relation" not in data
                    or data.get("relation") in {"is_a", "part_of"}
                ):
                    parents.add(parent)
    ...
    def process_goa_file(self, goa_path: Path) -> Dict[str, Dict[str, GOTerm]]:
        ...
        try:
            ...
            self.logger.info(
                f"Building normalized gene symbol map for {len(valid_gene_list)} genes"
            )
            gene_matches = {normalize_gene_symbol(g): g for g in valid_gene_list if g}

            with gzip.open(goa_path, "rt") as f:
                for line in tqdm_with_logging(
                    f, desc="Processing GOA entries", module_name="etl.go_terms"
                ):
                    if line.startswith("!"):
                        continue
                    ...
                    )  # DB:Reference field (where PMIDs are)
                    evidence = (
                        fields[6] if len(fields) > 6 else ""
                    )  # Evidence code (IEA, IPI, etc.)
    ...
    def _extract_special_terms(
        ...
        for go_id, term_data in go_terms.items():
            ...
            if (
                not isinstance(term_data, dict)
                or "aspect" not in term_data
                or "term" not in term_data
            ):
                continue
    ...
    def extract_publication_references(
            ...
            if evidence_text and "PMID:" in evidence_text:
                ...
                for extracted_pmid in pmids:
                    ...
                    if extracted_pmid != pmid:
                        publication = (
                            PublicationsProcessor.create_publication_reference(
                                pmid=extracted_pmid,
        ...
        if pmids_found > 0:
            self.logger.debug(
                f"Extracted {pmids_found} PMIDs from {len(go_terms)} GO terms"
            )
    ...
    def populate_initial_terms(self) -> None:
        ...
        try:
            ...
            progress_bar = get_progress_bar(
                len(gene_go_terms), "Updating GO terms", "etl.go_terms"
            )
            for i, (gene_symbol, go_terms) in enumerate(gene_go_terms.items()):
                if go_terms:
                    ...
                    (
                        molecular_functions,
                        cellular_locations,
                    ) = self._extract_special_terms(go_terms)
    ...
    def _update_go_terms_batch(self, updates: List[Tuple]) -> None:
        ...
        try:
            ...
            with self.get_db_transaction() as transaction:
                ...
                self.logger.debug(
                    "Updated source_references with GO term publications for backward compatibility"
                )
    ...
    def enrich_transcripts(self) -> None:
        ...
        try:
            ...
            progress_bar = get_progress_bar(
                len(transcripts), "Enriching GO terms", "etl.go_terms"
            )
            for i, (transcript_id, gene_symbol, go_terms) in enumerate(transcripts):
                ...
                molecular_functions, cellular_locations = self._extract_special_terms(
                    enriched_terms
                )
    ...
    def _update_enriched_terms_batch(self, updates: List[Tuple]) -> None:
        ...
        try:
            ...
            with self.get_db_transaction() as transaction:
                ...
                self.logger.debug(
                    f"Enriched batch of {len(updates)} genes (legacy UPDATE skipped)"
                )
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                "GO term enrichment with ancestors skipped (normalized schema migration)"
            )
