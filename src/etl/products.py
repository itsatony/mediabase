...
class ProductClassifier(BaseProcessor):
    ...
    def parse_uniprot_data(self, uniprot_file: Path) -> Dict[str, Dict[str, Any]]:
        ...
        try:
                ...
                for line in tqdm(f, total=line_count, desc="Parsing UniProt data"):
                    ...
                    if id_type == "DOMAIN" or id_type == "REGION" or id_type == "MOTIF":
                        current_entry["features"][
                            f"{id_type.lower()}_{len(current_entry['features'])}"
                        ] = value
                    ...
                    if id_type == "GO" and ":" in value:
                        ...
                        if len(go_parts) >= 1:
                            ...
                            if go_id.startswith("GO:"):
                                go_desc = (
                                    go_parts[1].strip() if len(go_parts) > 1 else ""
                                )
                    ...
                    if current_entry["gene_symbol"] and (
                        id_type == "Gene_Name" or stats["total_entries"] % 1000 == 0
                    ):
            ...
            self.logger.info(
                f"- Entries with gene symbols: {stats['with_gene_symbol']:,}"
            )
            ...
            self.logger.info(
                f"- Entries with function descriptions: {stats['with_function']:,}"
            )
    ...
    def update_gene_features(self, gene_data: Dict[str, Dict[str, Any]]) -> None:
        ...
        try:
            self.logger.info(
                f"Updating {len(gene_data):,} genes with features and GO terms"
            )
            ...
            self.logger.info(
                f"Found {len(overlap_symbols):,} overlapping genes to update"
            )
            ...
            for gene_symbol in tqdm(overlap_symbols, desc="Preparing feature updates"):
                ...
                # Add to updates
                updates.append(
                    (gene_symbol, json.dumps(features), json.dumps(go_terms), keywords)
                )
            ...
            self.logger.info(
                f"Updated {updated_genes:,} genes with features and GO terms"
            )
            ...
            try:
                if self.db_manager.cursor:
                    self.db_manager.cursor.execute(
                        "DROP TABLE IF EXISTS temp_gene_features"
                    )
                if self.db_manager.conn:
    ...
    def _update_feature_batch(
        ...
        try:
            ...
            self.logger.debug(
                f"Processed batch of {len(updates)} genes (legacy UPDATE skipped)"
            )
    ...
    def classify_gene(self, gene_data: Dict[str, Any]) -> List[str]:
        ...
        if function_text and isinstance(function_text, str):
            ...
            if any(
                term in function_text for term in ["phosphatase", "dephosphorylation"]
            ):
                product_types.add("phosphatase")

            if any(
                term in function_text
                for term in ["protease", "peptidase", "proteolytic"]
            ):
        ...
        if function_text and isinstance(function_text, str):
            ...
            for pmid in pmids:
                publication = PublicationsProcessor.create_publication_reference(
                    pmid=pmid, evidence_type="function_description", source_db="UniProt"
                )
...
class ProductProcessor(BaseProcessor):
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        if self.process_limit:
            try:
                ...
                self.logger.warning(
                    f"Invalid process_limit value: {self.process_limit}, processing all genes"
                )
    ...
    def get_genes_with_features(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            raise DatabaseError(
                f"Failed to retrieve genes for product classification: {e}"
            )

    def classify_genes(
        self, genes: List[Dict[str, Any]]
    ) -> List[Tuple[str, List[str], List[Publication]]]:
    ...
    def update_gene_types(
        self, classified_genes: List[Tuple[str, List[str], List[Publication]]]
    ) -> None:
        ...
        try:
            ...
            matched_genes = match_genes_bulk(
                classified_symbols, db_genes, use_fuzzy=True
            )
            ...
            with tqdm(total=total_genes, desc="Updating gene types") as pbar:
                ...
                for i in range(0, total_genes, batch_size):
                    ...
                    with self.get_db_transaction() as transaction:
                        ...
                        batch_data = [
                            (gene, types, json.dumps(pubs))
                            for gene, (types, pubs) in gene_data.items()
                        ]
                        ...
                        try:
                            ...
                            self.logger.warning(
                                f"Error inserting batch data, retrying individual records: {e}"
                            )
                            ...
                            for gene_symbol, product_types, publications in batch_data:
                                try:
                                    ...
                                    self.logger.warning(
                                        f"Skipping gene {gene_symbol} due to error: {record_error}"
                                    )
                                    continue
...
def integrate_products(self, product_data: Dict[str, Dict[str, Any]]) -> None:
    ...
    try:
        ...
        self.logger.info(
            f"Successfully integrated product data for {processed} transcripts"
        )
