...
class DemoDatasetGenerator:
    ...
    ONCOGENES = {
        'MYC': {'base_fold': 4.5, 'range': (3.0, 8.0), 'pathways': ['cell_cycle', 'proliferation']},
        'ERBB2': {'base_fold': 6.0, 'range': (4.0, 12.0), 'pathways': ['growth_signaling', 'her2']},
        'EGFR': {'base_fold': 5.5, 'range': (3.5, 9.0), 'pathways': ['growth_signaling', 'egfr']},
        'KRAS': {'base_fold': 3.8, 'range': (2.5, 6.5), 'pathways': ['ras_signaling', 'proliferation']},
        'PIK3CA': {'base_fold': 3.2, 'range': (2.0, 5.5), 'pathways': ['pi3k_akt', 'growth']},
        'AKT1': {'base_fold': 2.8, 'range': (2.0, 4.5), 'pathways': ['pi3k_akt', 'survival']},
        'CCND1': {'base_fold': 4.2, 'range': (2.5, 7.0), 'pathways': ['cell_cycle', 'g1_s']},
        'MDM2': {'base_fold': 3.5, 'range': (2.0, 6.0), 'pathways': ['p53_pathway', 'apoptosis_inhibition']},
        'BRAF': {'base_fold': 3.0, 'range': (2.0, 5.0), 'pathways': ['mapk', 'proliferation']},
        'NRAS': {'base_fold': 2.8, 'range': (1.8, 4.5), 'pathways': ['ras_signaling', 'proliferation']},
    }

    TUMOR_SUPPRESSORS = {
        'TP53': {'base_fold': 0.35, 'range': (0.15, 0.65), 'pathways': ['p53_pathway', 'apoptosis', 'dna_damage']},
        'RB1': {'base_fold': 0.45, 'range': (0.20, 0.70), 'pathways': ['cell_cycle', 'g1_s_checkpoint']},
        'PTEN': {'base_fold': 0.40, 'range': (0.15, 0.65), 'pathways': ['pi3k_akt', 'tumor_suppression']},
        'BRCA1': {'base_fold': 0.30, 'range': (0.10, 0.55), 'pathways': ['dna_repair', 'homologous_recombination']},
        'BRCA2': {'base_fold': 0.32, 'range': (0.12, 0.58), 'pathways': ['dna_repair', 'homologous_recombination']},
        'CDKN2A': {'base_fold': 0.25, 'range': (0.10, 0.50), 'pathways': ['cell_cycle', 'p16_pathway']},
        'CDKN1A': {'base_fold': 0.55, 'range': (0.30, 0.80), 'pathways': ['cell_cycle', 'p21_pathway']},
        'CDKN1B': {'base_fold': 0.48, 'range': (0.25, 0.75), 'pathways': ['cell_cycle', 'p27_pathway']},
        'APC': {'base_fold': 0.38, 'range': (0.15, 0.65), 'pathways': ['wnt_signaling', 'tumor_suppression']},
        'VHL': {'base_fold': 0.42, 'range': (0.20, 0.70), 'pathways': ['hypoxia_response', 'tumor_suppression']},
    }

    DNA_REPAIR_GENES = {
        'ATM': {'base_fold': 0.45, 'range': (0.25, 0.70), 'pathways': ['dna_damage_response', 'atm_checkpoint']},
        'CHEK1': {'base_fold': 0.52, 'range': (0.30, 0.75), 'pathways': ['cell_cycle_checkpoint', 'dna_repair']},
        'CHEK2': {'base_fold': 0.48, 'range': (0.28, 0.72), 'pathways': ['cell_cycle_checkpoint', 'dna_repair']},
        'RAD51': {'base_fold': 0.55, 'range': (0.35, 0.80), 'pathways': ['homologous_recombination', 'dna_repair']},
        'PARP1': {'base_fold': 0.60, 'range': (0.40, 0.85), 'pathways': ['base_excision_repair', 'parp_pathway']},
        'MLH1': {'base_fold': 0.35, 'range': (0.15, 0.60), 'pathways': ['mismatch_repair', 'msi']},
        'MSH2': {'base_fold': 0.40, 'range': (0.20, 0.65), 'pathways': ['mismatch_repair', 'msi']},
        'XRCC1': {'base_fold': 0.58, 'range': (0.35, 0.80), 'pathways': ['base_excision_repair', 'dna_repair']},
    }

    HORMONE_RECEPTORS = {
        'ESR1': {'base_fold': 2.5, 'range': (0.3, 6.0), 'pathways': ['estrogen_signaling', 'hormone_response']},
        'PGR': {'base_fold': 2.2, 'range': (0.2, 5.5), 'pathways': ['progesterone_signaling', 'hormone_response']},
        'AR': {'base_fold': 1.8, 'range': (0.3, 4.5), 'pathways': ['androgen_signaling', 'hormone_response']},
    }
    ...
    CANCER_SIGNATURES = {
        ...
        'lung_egfr': {
            ...
            'oncogene_multipliers': {'EGFR': 2.0, 'KRAS': 0.8, 'BRAF': 1.2},  # EGFR high, KRAS often wild-type
            'suppressor_multipliers': {'TP53': 0.6, 'RB1': 0.7},
            'target_genes': 300,
        },
        'colorectal_msi': {
            'dominant_pathways': ['mismatch_repair', 'immune_response', 'wnt_signaling'],
    ...
    def load_database_gene_info(self):
        ...
        try:
            ...
            for row in cursor.fetchall():
                gene_symbol, transcript_id, pathways, product_type, molecular_functions, drugs = row
            ...
            all_cancer_genes = set(self.ONCOGENES.keys()) | set(self.TUMOR_SUPPRESSORS.keys()) | set(self.DNA_REPAIR_GENES.keys()) | set(self.HORMONE_RECEPTORS.keys())
            self.stats['available_cancer_genes'] = len(all_cancer_genes & set(self.available_genes.keys()))
            self.stats['genes_with_pathways'] = len(self.gene_pathways)
            self.stats['genes_with_drugs'] = len(self.gene_drugs)

            console.print(f"[green]✓ Loaded information for {self.stats['total_genes_in_db']} genes[/green]")
            console.print(f"  • Cancer genes available: {self.stats['available_cancer_genes']}")
            console.print(f"  • Genes with pathways: {self.stats['genes_with_pathways']}")
    ...
    def generate_cancer_dataset(self, cancer_type: str, output_path: Path) -> Dict:
        ...
        if cancer_type not in self.CANCER_SIGNATURES:
            raise ValueError(f"Unknown cancer type: {cancer_type}. Available: {list(self.CANCER_SIGNATURES.keys())}")

        signature = self.CANCER_SIGNATURES[cancer_type]
        console.print(f"[blue]Generating {cancer_type} dataset with {signature['target_genes']} genes...[/blue]")

        dataset_rows = []
        gene_categories = {'oncogenes': 0, 'tumor_suppressors': 0, 'dna_repair': 0, 'hormone_receptors': 0, 'pathway_genes': 0, 'drug_targets': 0}
        ...
        for gene_symbol, gene_info in self.ONCOGENES.items():
            if gene_symbol in self.available_genes:
                fold_change = self._calculate_cancer_specific_fold_change(gene_symbol, gene_info, signature, 'oncogene')
                transcript_id = random.choice(self.available_genes[gene_symbol])

                row = self._create_gene_row(gene_symbol, transcript_id, fold_change, cancer_type, 'oncogene')
        ...
        for gene_symbol, gene_info in self.TUMOR_SUPPRESSORS.items():
            if gene_symbol in self.available_genes:
                fold_change = self._calculate_cancer_specific_fold_change(gene_symbol, gene_info, signature, 'tumor_suppressor')
                transcript_id = random.choice(self.available_genes[gene_symbol])

                row = self._create_gene_row(gene_symbol, transcript_id, fold_change, cancer_type, 'tumor_suppressor')
                dataset_rows.append(row)
                processed_genes.add(gene_symbol)
        ...
        for gene_symbol, gene_info in self.DNA_REPAIR_GENES.items():
            if gene_symbol in self.available_genes:
                fold_change = self._calculate_cancer_specific_fold_change(gene_symbol, gene_info, signature, 'dna_repair')
                transcript_id = random.choice(self.available_genes[gene_symbol])

                row = self._create_gene_row(gene_symbol, transcript_id, fold_change, cancer_type, 'dna_repair')
                dataset_rows.append(row)
                processed_genes.add(gene_symbol)
        ...
        for gene_symbol, gene_info in self.HORMONE_RECEPTORS.items():
            if gene_symbol in self.available_genes:
                ...
                if gene_symbol in signature.get('hormone_status', {}):
                    ...
                    fold_change = self._calculate_cancer_specific_fold_change(gene_symbol, gene_info, signature, 'hormone_receptor')

                transcript_id = random.choice(self.available_genes[gene_symbol])
                row = self._create_gene_row(gene_symbol, transcript_id, fold_change, cancer_type, 'hormone_receptor')
                dataset_rows.append(row)
                processed_genes.add(gene_symbol)
        ...
        if remaining_needed > 0:
            pathway_genes, drug_genes = self._select_additional_genes(signature, remaining_needed, processed_genes)

            for gene_symbol, transcript_id, fold_change, gene_type in pathway_genes + drug_genes:
                row = self._create_gene_row(gene_symbol, transcript_id, fold_change, cancer_type, gene_type)
                dataset_rows.append(row)
        ...
        stats = {
            ...
            'fold_change_range': {
                ...
                'mean': sum(row['cancer_fold'] for row in dataset_rows) / len(dataset_rows)
            },
            'upregulated_genes': len([r for r in dataset_rows if r['cancer_fold'] > 1.5]),
            'downregulated_genes': len([r for r in dataset_rows if r['cancer_fold'] < 0.67]),
            'output_path': str(output_path)
        }

        return stats

    def _calculate_cancer_specific_fold_change(self, gene_symbol: str, gene_info: Dict, signature: Dict, gene_type: str) -> float:
        ...
        if gene_type == 'oncogene' and gene_symbol in signature.get('oncogene_multipliers', {}):
        ...
        elif gene_type == 'tumor_suppressor' and gene_symbol in signature.get('suppressor_multipliers', {}):
    ...
    def _select_additional_genes(self, signature: Dict, count_needed: int, processed_genes: Set[str]) -> Tuple[List, List]:
        ...
        for gene_symbol in available_genes:
            if gene_symbol in self.gene_pathways:
                ...
                if any(pathway in str(gene_pathways).lower() for pathway in ['cancer', 'tumor', 'cell_cycle', 'apoptosis', 'dna', 'repair', 'growth', 'proliferation']):
        ...
        selected_pathway = random.sample(pathway_candidates, pathway_count) if pathway_candidates else []
        for gene_symbol in selected_pathway:
            ...
            pathway_genes.append((gene_symbol, transcript_id, fold_change, 'pathway_gene'))

        # Select drug target genes
        selected_drugs = random.sample(drug_candidates, drug_count) if drug_candidates else []
    ...
    def _create_gene_row(self, gene_symbol: str, transcript_id: str, fold_change: float, cancer_type: str, gene_category: str) -> Dict:
    ...
    def _save_dataset(self, dataset_rows: List[Dict], output_path: Path, cancer_type: str):
    ...
    def generate_all_datasets(self, output_dir: Path) -> Dict:
        ...
        for cancer_type in self.CANCER_SIGNATURES.keys():
            ...
            try:
                ...
                console.print(f"[green]✓ Generated {cancer_type}: {stats['total_genes']} genes[/green]")
            except Exception as e:
    ...
    def _generate_summary_report(self, all_stats: Dict, output_dir: Path):
        ...
        report = {
            ...
            'total_genes_generated': sum(stats['total_genes'] for stats in all_stats.values()),
        }
...
def main():
    ...
    parser.add_argument(
        '--all',
        action='store_true',
        help='Generate all cancer type datasets'
    )

    parser.add_argument(
        '--cancer-type',
        choices=['breast_her2', 'breast_tnbc', 'lung_egfr', 'colorectal_msi', 'pancreatic_pdac', 'comprehensive'],
