...
class TestDESeq2FormatDetection:
    ...
    def test_detect_deseq2_format_valid(
        self, tmp_path: Path, db_config: Dict[str, Any], deseq2_csv_data: pd.DataFrame
    ):
    ...
    def test_detect_deseq2_format_variations(
        self,
        tmp_path: Path,
        db_config: Dict[str, Any],
        deseq2_csv_variations: List[pd.DataFrame],
    ):
        ...
        for i, variation_data in enumerate(deseq2_csv_variations):
            ...
            ), f"Failed to detect DESeq2 format in variation {i}"

    def test_detect_deseq2_format_standard_format(
        self, tmp_path: Path, db_config: Dict[str, Any], standard_csv_data: pd.DataFrame
    ):
    ...
    def test_log2_to_linear_conversion(self, tmp_path: Path, db_config: Dict[str, Any]):
        ...
        test_data = pd.DataFrame(
            {
                ...
                "log2FoldChange": [
                    ...
                    2.0,
                    0.0,
                ],  # Should convert to 2.0, 0.5, 4.0, 1.0
        ...
        for transcript_id, expected_fold in expected_values.items():
            ...
            assert (
                abs(creator.transcript_updates[transcript_id] - expected_fold) < 0.001
            )
...
class TestGeneSymbolMapping:
    ...
    def test_gene_symbol_mapping_success(
        self, tmp_path: Path, db_config: Dict[str, Any], mock_gene_mappings: List[tuple]
    ):
    ...
    def test_gene_symbol_mapping_database_error(
        ...
        with patch("scripts.create_patient_copy.get_db_manager") as mock_db_manager:
            ...
            with pytest.raises(
                CSVValidationError, match="Failed to connect to source database"
            ):
                creator._load_gene_symbol_mapping()

    def test_process_deseq2_data_with_unmapped_symbols(
        self, tmp_path: Path, db_config: Dict[str, Any]
    ):
...
class TestDESeq2Integration:
    ...
    def test_end_to_end_deseq2_workflow(
        self, tmp_path: Path, db_config: Dict[str, Any]
    ):
        ...
        with patch("scripts.create_patient_copy.get_db_manager") as mock_db_manager:
            ...
            assert (
                abs(creator.transcript_updates["ENST00000357654"] - (2**0.61)) < 0.01
            )  # BRCA1
            assert (
                abs(creator.transcript_updates["ENST00000269305"] - (2**-2.62)) < 0.01
            )  # TP53
    ...
    def test_deseq2_vs_standard_format_detection(
        self, tmp_path: Path, db_config: Dict[str, Any]
    ):
    ...
    def test_edge_cases_and_error_handling(
        ...
        with patch("pandas.DataFrame.dropna") as mock_dropna:
            ...
            creator._process_deseq2_data(
                mock_dropna.return_value, "SYMBOL", "log2FoldChange"
            )
