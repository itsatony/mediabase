...
class TestDESeq2CoreFunctionality:
    ...
    def test_deseq2_format_detection_positive_cases(self, tmp_path: Path, db_config: Dict[str, Any]):
        ...
        for i, columns in enumerate(test_cases):
            ...
            assert len(deseq2_indicators) == 2, f"Failed to detect DESeq2 format in case {i}: {columns.keys()}"
            assert 'symbol' in deseq2_indicators
            assert 'log2_fold_change' in deseq2_indicators
    
    def test_deseq2_format_detection_negative_cases(self, tmp_path: Path, db_config: Dict[str, Any]):
        ...
        for i, columns in enumerate(negative_cases):
            ...
            assert len(deseq2_indicators) < 2, f"Incorrectly detected DESeq2 format in negative case {i}: {columns.keys()}"
    ...
    def test_log2_to_linear_conversion_mathematics(self):
        ...
        for log2_value, expected_linear in test_cases:
            ...
            assert abs(linear_result - expected_linear) < 0.01, f"Log2 conversion failed: 2^{log2_value} = {linear_result}, expected ≈ {expected_linear}"
    
    def test_deseq2_column_mapping_logic(self, tmp_path: Path, db_config: Dict[str, Any]):
    ...
    def test_process_deseq2_data_without_database(self, tmp_path: Path, db_config: Dict[str, Any]):
        ...
        for transcript_id, expected_value in expected_conversions.items():
            ...
            assert abs(actual_value - expected_value) < 0.001, f"Conversion error: {transcript_id} = {actual_value}, expected {expected_value}"
    ...
    def test_column_name_case_insensitivity(self, tmp_path: Path, db_config: Dict[str, Any]):
        ...
        for i, columns in enumerate(case_variations):
            ...
            assert len(deseq2_indicators) >= 1, f"Case-insensitive detection failed for case {i}: {columns.keys()}"
        ...
        for log2_val, expected_min in edge_cases:
            if not np.isinf(log2_val):
                ...
                if np.isfinite(expected_min):
                    assert abs(linear_result - expected_min) < 0.001 or linear_result == expected_min
    
    def test_multiple_transcript_same_gene_handling(self, tmp_path: Path, db_config: Dict[str, Any]):
    ...
    def test_standard_vs_deseq2_format_distinction(self, tmp_path: Path, db_config: Dict[str, Any]):
        ...
        assert len(deseq2_indicators1) < 2, "Standard format incorrectly detected as DESeq2"
