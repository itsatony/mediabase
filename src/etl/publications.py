...
DEFAULT_CACHE_TTL = (
    2592000  # 30 days in seconds for publications (much longer than other data)
)
...
class PublicationsProcessor(BaseProcessor):
    ...
    def _retry_with_backoff(
        self, func: Any, *args: Any, operation_name: str = "API request", **kwargs: Any
    ) -> Optional[Any]:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if status_code and 400 <= status_code < 500 and status_code != 429:
                    self.logger.error(
                        f"{operation_name} failed with status {status_code}: {e}"
                    )
                    return None
            ...
            for i in range(0, len(valid_pmids), PUBMED_BATCH_SIZE):
                ...
                try:
                    ...
                    for pmid in batch:
                        self.failed_pmids.add(pmid)
                    self.logger.warning(
                        f"Error fetching batch {i}-{i+PUBMED_BATCH_SIZE}: {e}"
                    )
                    continue
    ...
    def _update_publication_references(
        self, pub_metadata: Dict[str, Dict[str, Any]]
    ) -> None:
        ...
        try:
            self.logger.info(
                f"Updating {len(pub_metadata)} publications in the database"
            )
            ...
            with tqdm(total=row_count, desc="Enriching references in database") as pbar:
                for section in sections:
                    ...
                    for transcript_id, refs in self.db_manager.cursor.fetchall():
                        ...
                        if modified:
                            updates.append(
                                (json.dumps(refs_list), section, transcript_id)
                            )
                            records_enriched += 1
            ...
            self.logger.info(
                f"Enrichment completed. Enriched {records_enriched} records. Processed {updates_counter} updates."
            )
    ...
    def _execute_publication_updates(
        self, updates: List[Tuple[str, str, str]], section: str
    ) -> None:
        ...
        try:
            ...
            self.execute_batch(
                query, [(refs, transcript_id) for refs, _, transcript_id in updates]
            )
    ...
    def enrich_publications_bulk(
        ...
        try:
            self.logger.info(
                f"Enriching {len(publications)} publications with PubMed metadata"
            )
            ...
            for pmid, metadata in pub_metadata.items():
                if pmid in pmid_to_publication:
                    ...
                    if metadata.get("citation_count") and not publication.get(
                        "citation_count"
                    ):
            ...
            self.logger.info(
                f"Successfully enriched {enriched_count}/{len(pmids)} publications"
            )
            return publications
    ...
    def generate_production_report(self) -> Dict[str, Any]:
        ...
        if self.fetch_errors:
            error_log_file = (
                self.pub_dir
                / f"fetch_errors_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
            try:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if len(self.failed_pmids) > 0:
                failure_rate = (
                    len(self.failed_pmids)
                    / max(1, self.pmids_fetched + len(self.failed_pmids))
                ) * 100
                if failure_rate > 10:
                    self.logger.warning(
    ...
    def integrate_publications(
        self, publications_data: Dict[str, List[Dict[str, Any]]]
    ) -> None:
        ...
        try:
            ...
            for gene_symbol, publications in publications_data.items():
                ...
                updates.append(
                    (gene_symbol, uniprot_ids, ncbi_ids, json.dumps(publication_refs))
                )
                ...
                if len(updates) >= self.batch_size:
                    ...
                    updates = []
                    self.logger.info(
                        f"Processed {processed} genes with publication data"
                    )
            ...
            self.logger.info(
                f"Successfully integrated publication data for {processed} genes"
            )
    ...
    def _update_publication_batch(
        self, updates: List[Tuple[str, List[str], List[str], str]]
    ) -> None:
