...
class ScoringAnalyticsProcessor(BaseProcessor):
    ...
    def analyze_gene_evidence_profile(
        self, gene_symbol: str
    ) -> Optional[ScoringAnalytics]:
        ...
        try:
            ...
            self.logger.error(
                f"Error analyzing evidence profile for {gene_symbol}: {e}"
            )
            return None
    ...
    def _calculate_analytics_metrics(
        ...
        evidence_diversity_score = min(
            len(evidence_sources) / 6.0, 1.0
        )  # Normalize to max 6 sources
        ...
        clinical_strength = (
            statistics.mean(component_scores.get("clinical", [0])) / 30.0
        )  # Normalize to 0-1
        mechanistic_depth = (
            statistics.mean(component_scores.get("mechanistic", [0])) / 25.0
        )
        publication_support = (
            statistics.mean(component_scores.get("publication", [0])) / 20.0
        )
        ...
        if len(all_scores) > 1:
            ...
            cross_validation_score = max(
                0, 1.0 - (score_variance / 1000)
        ...
        use_case_rankings = dict(
            sorted(use_case_scores.items(), key=lambda x: x[1], reverse=True)
        )
    ...
    def _identify_evidence_gaps(
        ...
        if diversity < 0.5:
            gaps.append(
                "Limited evidence source diversity - consider additional databases"
            )

        if clinical < 0.3:
            gaps.append(
                "Weak clinical evidence - search for clinical trials and FDA approvals"
            )

        if mechanistic < 0.4:
            gaps.append(
                "Limited mechanistic understanding - investigate pathway involvement"
            )
        ...
        if genomic < 0.2:
            gaps.append(
                "Minimal genomic evidence - explore mutation and biomarker associations"
            )
    ...
    def _generate_recommendations(
        ...
        if priority_areas:
            recommendations.append(
                f"High priority for {', '.join(priority_areas)} research"
            )
        ...
        if use_case_rankings:
            ...
            if best_use_case[1] >= 60:
                recommendations.append(
                    f"Strongly recommended for {best_use_case[0].replace('_', ' ')} (score: {best_use_case[1]:.1f})"
                )
            elif best_use_case[1] >= 40:
                recommendations.append(
                    f"Consider for {best_use_case[0].replace('_', ' ')} with additional validation"
                )

        # Clinical development recommendations
        if clinical_strength >= 0.7:
            recommendations.append(
                "Ready for clinical development - strong evidence base"
            )
        elif clinical_strength >= 0.4:
            recommendations.append(
                "Suitable for preclinical development - gather more clinical evidence"
            )
        else:
            recommendations.append(
                "Requires significant preclinical work before clinical development"
            )

        # Research priority recommendations
        if confidence >= 0.8:
            recommendations.append(
                "High confidence target - prioritize for resource allocation"
            )
        elif confidence >= 0.6:
            recommendations.append(
                "Moderate confidence - validate with additional evidence"
            )
        else:
        ...
        if len(evidence_gaps) <= 2:
            recommendations.append(
                "Well-characterized target with comprehensive evidence"
            )
        elif len(evidence_gaps) <= 4:
            recommendations.append(
                "Good evidence base - address specific gaps identified"
            )
        else:
            recommendations.append(
                "Requires comprehensive evidence gathering across multiple dimensions"
            )
    ...
    def generate_comparative_analysis(self, gene_list: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            comparison = {
                ...
                "clinical_readiness_assessment": self._assess_clinical_readiness(
                    gene_analytics
                ),
                "research_priorities": self._prioritize_research_opportunities(
                    gene_analytics
                ),
    ...
    def _rank_genes_by_metrics(
        self, gene_analytics: Dict[str, ScoringAnalytics]
        ...
        clinical_ranking = sorted(
            gene_analytics.items(), key=lambda x: x[1].clinical_strength, reverse=True
        )
    ...
    def _compare_use_cases(
        ...
        for use_case, data in use_case_data.items():
            ...
            comparison[use_case] = {
                ...
                "std_score": round(
                    statistics.stdev(scores) if len(scores) > 1 else 0, 2
                ),
    ...
    def _analyze_evidence_quality(
        ...
        confidence_scores = [
            analytics.recommendation_confidence for analytics in gene_analytics.values()
        ]
        diversity_scores = [
            analytics.evidence_diversity_score for analytics in gene_analytics.values()
        ]
        evidence_counts = [
            analytics.total_evidence_items for analytics in gene_analytics.values()
        ]
    ...
    def _prioritize_research_opportunities(
        self, gene_analytics: Dict[str, ScoringAnalytics]
    ) -> List[Dict[str, Any]]:
        ...
        for gene, analytics in gene_analytics.items():
            ...
            potential_score = (
                max(analytics.use_case_rankings.values())
    ...
    def _generate_portfolio_recommendations(
        ...
        high_confidence_genes = len(
            [a for a in gene_analytics.values() if a.recommendation_confidence >= 0.8]
        )
        clinical_ready_genes = len(
            [a for a in gene_analytics.values() if a.clinical_strength >= 0.7]
        )

        # Portfolio composition recommendations
        if high_confidence_genes / total_genes >= 0.3:
            recommendations.append(
                "Strong portfolio with high proportion of confident targets"
            )
        else:
            recommendations.append(
                "Portfolio needs more high-confidence targets - focus on evidence gathering"
            )

        if clinical_ready_genes / total_genes >= 0.2:
            recommendations.append(
                "Good clinical pipeline potential - prioritize clinical development"
            )
        else:
            recommendations.append(
                "Limited clinical readiness - invest in preclinical development"
            )
        ...
        for analytics in gene_analytics.values():
            if analytics.use_case_rankings:
                best_use_case = max(
                    analytics.use_case_rankings.items(), key=lambda x: x[1]
                )[0]
                use_case_strengths[best_use_case] += 1

        dominant_use_case = (
            max(use_case_strengths.items(), key=lambda x: x[1])
            if use_case_strengths
            else None
        )
        if dominant_use_case and dominant_use_case[1] / total_genes > 0.5:
            recommendations.append(
                f"Portfolio heavily weighted toward {dominant_use_case[0]} - consider diversification"
            )
        else:
        ...
        priority_gene_count = len(
            [
                ...
                for gene, analytics in gene_analytics.items()
                if any(
                    gene in genes for genes in self.cancer_research_priorities.values()
                )
            ]
        )
        ...
        if priority_gene_count / total_genes >= 0.4:
            ...
            recommendations.append(
                "Consider including more high-priority cancer research targets"
            )
