...
class EvidenceScoringProcessor(BaseProcessor):
    ...
    def calculate_clinical_evidence_score(
        ...
        if chembl_data:
            ...
            for trial in trials:
                ...
                metadata["components"].append(
                    {"type": "chembl_trial", "score": phase_score, "phase": phase}
                )
        ...
        if repurposing_data:
            ...
            metadata["components"].append(
                {"type": "repurposing_hub", "score": phase_score, "phase": phase}
            )
    ...
    def calculate_mechanistic_evidence_score(
        self, gene_data: Dict[str, Any]
    ) -> EvidenceScore:
        ...
        if pharmgkb_pathways:
            ...
            cancer_relevant_pathways = sum(
                1
                for pathway in pharmgkb_pathways.values()
    ...
    def calculate_publication_evidence_score(
        self, gene_data: Dict[str, Any]
    ) -> EvidenceScore:
    ...
    def calculate_genomic_evidence_score(
        self, gene_data: Dict[str, Any]
    ) -> EvidenceScore:
    ...
    def calculate_safety_evidence_score(
        self, gene_data: Dict[str, Any]
    ) -> EvidenceScore:
    ...
    def calculate_composite_score(
        self, evidence_scores: List[EvidenceScore], use_case: UseCase
    ) -> CompositeScore:
    ...
    def process_evidence_scoring(
            ...
            for record in records:
                ...
                for drug_key, drug_data in gene_data["drugs"].items():
                    if isinstance(drug_data, dict):
                        ...
                        score = use_case_scores[UseCase.THERAPEUTIC_TARGETING.value][
                            "overall_score"
                        ]
        ...
        cursor.executemany(
            update_query, [(scores, tid) for tid, scores in score_updates]
        )
    ...
    def _generate_scoring_statistics(
        self, score_updates: List[Tuple[str, str]]
    ) -> Dict[str, Any]:
        ...
        for use_case, scores in use_case_stats.items():
            if scores:
                stats["use_case_statistics"][use_case] = {
                    ...
                    "medium_confidence_genes": len(
                        [s for s in scores if 40 <= s <= 70]
                    ),
        ...
        try:
            ...
            if stats:
                ...
                for use_case, case_stats in use_case_stats.items():
                    ...
                    self.logger.info(
                        f"  - {use_case}: mean={mean_score:.1f}, high_confidence={high_conf}"
                    )
