...
class IDEnrichmentProcessor(BaseProcessor):
    ...
    def _filter_uniprot_mapping(self, input_path: Path) -> Path:
        ...
        if output_path.exists() and meta_path.exists():
            try:
                ...
                self.logger.warning(
                    f"Error reading filter metadata, will re-filter: {e}"
                )

        try:
            self.logger.info(
                "Filtering UniProt ID mapping to ensure human entries only"
            )
            ...
            with gzip.open(input_path, "rt") as f_in:
                ...
                for line in f_in:
                    ...
                    if (
                        len(parts) >= 3
                        and parts[1] == "NCBI_TaxID"
                        and parts[2] == HUMAN_TAXID
                    ):
                ...
                self.logger.info(
                    f"Second pass: extracting {len(human_uniprot_ids):,} human UniProt entries..."
                )
                ...
                for line in f:
                    ...
                    if id_type == "Gene_Name":
                        ...
                        if id_value.startswith("ENSG"):  # Ensembl gene ID
                            uniprot_mapping[uniprot_id]["ensembl_gene_id"].append(
                                id_value
                            )
                            id_stats["ensembl_ids"] += 1
                        elif id_value.startswith("ENST"):  # Ensembl transcript ID
                            uniprot_mapping[uniprot_id]["ensembl_transcript_id"].append(
                                id_value
                            )
                            id_stats["ensembl_ids"] += 1
                    elif id_type == "RefSeq":
                        if id_value.startswith("NP_") or id_value.startswith("XP_"):
                            uniprot_mapping[uniprot_id]["refseq_protein_id"].append(
                                id_value
                            )
                            id_stats["refseq_ids"] += 1
                        elif id_value.startswith("NM_") or id_value.startswith("XM_"):
                            uniprot_mapping[uniprot_id]["refseq_mrna_id"].append(
                                id_value
                            )
                            id_stats["refseq_ids"] += 1
                    elif id_type == "HGNC":
    ...
    def update_transcript_ids(
        self, gene_mapping: Dict[str, Dict[str, List[str]]]
    ) -> None:
        ...
        try:
            ...
            db_gene_symbols = {
                row[0] for row in self.db_manager.cursor.fetchall() if row[0]
            }
            self.logger.info(
                f"Found {len(db_gene_symbols):,} unique gene symbols in database"
            )
            ...
            for gene_symbol in tqdm(overlap_symbols, desc="Preparing ID updates"):
                ...
                if (
                    "ensembl_transcript_id" in mapping
                    and mapping["ensembl_transcript_id"]
                ):
    ...
    def _update_id_batch(self, updates: List[Tuple]) -> None:
        ...
        try:
            ...
            if sample_update:
                self.logger.debug(
                    f"Sample update - Gene: {sample_update[5]}, "
            ...
            if self.db_manager.conn and self.db_manager.cursor:
                ...
                try:
                        ...
                        if ncbi_ids:  # ncbi_ids is a list
                            for ncbi_id in ncbi_ids:
                                if ncbi_id:  # Skip empty strings
                                    ...
                                    cross_ref_inserts.append(
                                        ("GeneID", ncbi_id, gene_symbol)
                                    )

                    if cross_ref_inserts:
                        self.logger.info(
                            f"Inserting {len(cross_ref_inserts)} NCBI cross-references into gene_cross_references"
                        )
                        self.db_manager.cursor.executemany(
                            """
                    ...
                    if not self.db_manager.conn.autocommit:
                        ...
                        self.logger.debug(
                            f"Committed batch update of {len(updates)} records and {len(cross_ref_inserts)} cross-references"
                        )
    ...
    def download_gene2ensembl(self) -> Path:
        ...
        try:
            ...
            # Use the BaseProcessor download method
            mapping_file = self.download_file(
                url=self.gene2ensembl_url, file_path=self.id_dir / "gene2ensembl.gz"
            )
            return mapping_file
        except Exception as e:
    ...
    def download_hgnc_complete(self) -> Path:
        ...
        try:
            ...
            hgnc_file = self.download_file(
                url=self.hgnc_url, file_path=self.id_dir / "hgnc_complete.txt"
            )
            return hgnc_file
        except Exception as e:
        ...
        try:
            ...
            for gene_symbol, ensembl_gene_id in db_genes:
                ...
                if ensembl_gene_id in gene2ensembl_mapping:
                    ...
                    comprehensive_mapping[gene_symbol]["ncbi_id"].append(
                        gene2ensembl_data["ncbi_gene_id"]
                    )
                    if gene2ensembl_data["ensembl_transcript_id"]:
                        comprehensive_mapping[gene_symbol][
                            "ensembl_transcript_id"
                        ].append(gene2ensembl_data["ensembl_transcript_id"])
                    if gene2ensembl_data["rna_accession"]:
                        comprehensive_mapping[gene_symbol]["refseq_mrna_id"].append(
                            gene2ensembl_data["rna_accession"]
                        )
                    if gene2ensembl_data["protein_accession"]:
                        comprehensive_mapping[gene_symbol]["refseq_protein_id"].append(
                            gene2ensembl_data["protein_accession"]
                        )
            ...
            for stat_name, count in integration_stats.items():
                percentage = (
                    (count / integration_stats["total_genes"]) * 100
                    if integration_stats["total_genes"] > 0
                    else 0
                )
