...
from ..utils.publication_utils import (
    extract_pmids_from_text, extract_clinical_trial_ids_from_text, 
    format_pmid_url, format_publication_url
)
...
CANCER_RELATED_CONDITIONS = [
    ...
    "sarcoma", "lymphoma", "leukemia", "melanoma", "glioma", "adenocarcinoma",
    "oncology", "metastasis", "chemotherapy", "radiation therapy"
]
...
class ClinicalTrialsProcessor(BaseProcessor):
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        # API configuration
        self.api_base_url = config.get('clinical_trials_api_base', CLINICAL_TRIALS_API_BASE)
        self.rate_limit = config.get('clinical_trials_rate_limit', DEFAULT_RATE_LIMIT)
        self.max_results = config.get('clinical_trials_max_results', MAX_RESULTS_PER_REQUEST)
        
        # Processing options
        self.cancer_only = config.get('clinical_trials_cancer_only', True)
        self.include_completed_only = config.get('clinical_trials_completed_only', False)
        ...
        # Cache TTL
        self.cache_ttl = config.get('clinical_trials_cache_ttl', 7 * 24 * 60 * 60)  # 7 days
    ...
    def _make_api_request(self, endpoint: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise DownloadError(f"ClinicalTrials.gov API request failed for {endpoint}: {e}")
        except json.JSONDecodeError as e:
            raise DownloadError(f"Invalid JSON response from ClinicalTrials.gov API: {e}")
    ...
    def search_trials_by_gene(self, gene_symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if self.cancer_only:
                ...
                cancer_condition = " OR ".join([f'"{condition}"' for condition in CANCER_RELATED_CONDITIONS[:5]])
            ...
            if self.include_completed_only:
                ...
                params['query.status'] = 'COMPLETED,ACTIVE_NOT_RECRUITING,RECRUITING,ENROLLING_BY_INVITATION'
    ...
    def _extract_trial_data(self, study: Dict[str, Any], gene_symbol: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if self.cancer_only:
                is_cancer_related = any(
                    any(cancer_term.lower() in condition.lower() 
    ...
    def _extract_trial_publications(self, derived_section: Dict[str, Any], nct_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.warning(f"Failed to extract publications from trial {nct_id}: {e}")
        
        return publications

    def get_trials_for_genes(self, gene_symbols: List[str]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        self.logger.info(f"Found trials for {len(gene_trials)} genes, total API requests: {self.api_requests_made}")
        return gene_trials

    def extract_publication_references(self, gene_trials: Dict[str, List[Dict[str, Any]]]) -> List[Publication]:
            ...
            for trial in trials:
                ...
                if nct_id:
                    trial_ref = {
                        ...
                        'url': trial.get('url', format_publication_url(nct_id, 'clinical_trial'))
                    }
    ...
    def update_transcript_clinical_trial_data(self, gene_trials: Dict[str, List[Dict[str, Any]]]) -> None:
        ...
        try:
            ...
            self.logger.info(f"Found {len(matching_genes):,} genes in database that have clinical trial data")
            
            if not matching_genes:
                self.logger.warning("No gene overlap found between database and clinical trials")
                return
            ...
            for gene_symbol in matching_genes:
                ...
                trial_summary = {
                    ...
                    'phases': list(set(trial.get('phase', 'Unknown') for trial in trials)),
                    'statuses': list(set(trial.get('status', 'Unknown') for trial in trials)),
                    'conditions': list(set().union(*[trial.get('conditions', []) for trial in trials])),
                    'recent_trials': len([t for t in trials if self._is_recent_trial(t)]),
                    'completed_trials': len([t for t in trials if t.get('status') == 'COMPLETED']),
                    'active_trials': len([t for t in trials if 'ACTIVE' in t.get('status', '') or 'RECRUITING' in t.get('status', '')])
                }
                
                # Build detailed trial records (limited to most relevant)
                detailed_trials = []
                for trial in sorted(trials, key=lambda x: (x.get('phase', ''), x.get('start_date', '')), reverse=True)[:10]:
                    detailed_trials.append({
                        'nct_id': trial.get('nct_id'),
                        'title': trial.get('title', '')[:200],  # Truncate long titles
                        'phase': trial.get('phase'),
                        'status': trial.get('status'),
                        'conditions': trial.get('conditions', [])[:5],  # Limit conditions
                        'start_date': trial.get('start_date'),
                        'completion_date': trial.get('completion_date'),
                        'lead_sponsor': trial.get('lead_sponsor'),
                        'url': trial.get('url')
            ...
            self.logger.info(f"Updating {len(update_data):,} transcript records with clinical trial data")
            ...
            self.logger.info(f"Successfully updated {updated_count:,} records with clinical trial data")
        ...
        try:
            ...
            if publications:
                ...
                if trial_publications:
                    self.logger.info(f"Processing {len(trial_publications)} clinical trial publication references")
