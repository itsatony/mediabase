...
logger = setup_logging(module_name=__name__, log_file="etl_pipeline.log")
...
def run_module(
    ...
    try:
        ...
        if reset_db:
            ...
            logger.info(
                f"Database reset complete with schema {LATEST_SCHEMA_VERSION} applied"
            )
        else:
            ...
            if current_version and current_version < LATEST_SCHEMA_VERSION:
                logger.info(
                    f"Upgrading database schema from {current_version} to {LATEST_SCHEMA_VERSION}"
                )
...
def run_pipeline(
    ...
    if modules and "id_enrichment" not in modules:
        ...
        logger.warning(
            "ID enrichment module is not included. This could affect other modules that rely on ID mappings."
        )
    ...
    try:
        ...
        if reset_db:
            ...
            logger.info(
                f"Database has been reset and schema {LATEST_SCHEMA_VERSION} applied"
            )
...
def get_modules_to_run(args) -> List[str]:
    ...
    if args.module:
        ...
        if "chembl_drugs" in requested_modules and "drugs" in requested_modules:
            ...
            logger.info(
                "Both 'drugs' and 'chembl_drugs' were specified. Using only 'chembl_drugs'"
            )
        ...
        if (
            "chembl_drugs" in requested_modules
            and "drugs" in modules_to_run
            and "chembl_drugs" not in modules_to_run
        ):
            # Replace drugs with chembl_drugs in the sequence
            modules_to_run = [
                m if m != "drugs" else "chembl_drugs" for m in modules_to_run
            ]
        ...
        logging.info(
            f"Running modules in optimal sequence: {', '.join(modules_to_run)}"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--limit-transcripts", type=int, help="Limit number of transcripts to process"
    )
    parser.add_argument(
        "--log-level",
    ...
    parser.add_argument(
        "--reset-db", action="store_true", help="Reset database before running pipeline"
    )
    parser.add_argument(
        "--force-download",
        action="store_true",
...
def main() -> int:  # Change return type to int for clarity
    ...
    parser.add_argument("--rate-limit", type=float, help="API rate limit in seconds")
    ...
    try:
        ...
        if args.reset_db:
            ...
            try:
                ...
                logger.info(
                    f"Database reset complete with schema {LATEST_SCHEMA_VERSION} properly applied"
                )
            except Exception as e:
